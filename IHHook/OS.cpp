...
int CheckVersionDelta(const unsigned long checkVersion[], std::string& exeVersionStr)
{
    ...
    spdlog::debug(L"GetModuleFileName: {}", exeName.c_str());
    ...
    DWORD exeVersion[4] = {HIWORD(dwFileVersionMS), LOWORD(dwFileVersionMS), HIWORD(dwFileVersionLS), LOWORD(dwFileVersionLS)};

    exeVersionStr = std::to_string(exeVersion[0]) + "," + std::to_string(exeVersion[1]) + "," + std::to_string(exeVersion[2]) + "," + std::to_string(exeVersion[3]);

    spdlog::info("mgsv exe version: {}", exeVersionStr);
    // GOTCHA: DEBUGNOW: will return wrong delta if they bump a sub version and
    // reset
    for (int i = 0; i < 4; i++)
    {
...
{
    // tex user having unicode path might be trouble, but on a quick test lua is
    // hinky with utf16
...
void GetAllWindowsFromProcessID(DWORD dwProcessID, std::vector<HWND>& vhWnds)
{
    ...
    do
    {
        ...
        if (dwWindowProcessID == dwProcessID)
        {
            ...
            GetWindowTextW(hCurWnd, &title[0],
                           static_cast<int>(title.size())); // note: C++11 only
...
// tex ASSUMPTION: Based on running GetAllWindowsFromProcessID, MGSV only has
// one window (well two if I compile IHHook to open a console while debugging).
// Don't want to test for the MGSV window title since I don't know if it's
// localized or not Also would fail if any other funky 3rd party programs that
// like to inject their own windows
HWND GetMainWindow()
{
    // find all hWnds (vhWnds) associated with a process id (dwProcessID)
    HWND hWnd = NULL;
