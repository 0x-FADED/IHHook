...
namespace IHHook
{
    namespace OS
    {
        ...
        int StartProcess(LPCWSTR lpApplicationPath, LPWSTR lpCommandLine)
        {
            ...
            int succeeded = CreateProcess(
                lpApplicationPath, // the path
        ...
        // tex ASSUMPTION: Based on running GetAllWindowsFromProcessID, MGSV only has one window (well two if I compile IHHook
        // to open a console while debugging). Don't want to test for the MGSV window title since I don't know if it's localized
        // or not Also would fail if any other funky 3rd party programs that like to inject their own windows
        HWND GetMainWindow()
        {
            // find all hWnds (vhWnds) associated with a process id (dwProcessID)
            HWND hWnd = NULL;
