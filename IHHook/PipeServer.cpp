...
namespace IHHook
{
    ...
    namespace PipeServer
    {
        ...
        void QueueMessageOut(std::string message)
        {
            ...
            // DEBUGNOW only add if pipe connected (but at moment we have no way of tracking since pipeserverthread is hands off
            // as far as launching a pipe
...
#define PSIZE 512
        ...
        DWORD WINAPI PipeServerThread(LPVOID lpvParam)
        {
            ...
            while (!doShutDown)
            {
                ...
                // only defined this to try and get around namedpipe client needs InOut even for server out/readonly pipe, but I
                // suppose having it defined rather than using default is better ultimately just want this for the pipe, but
                // cant hide it away in a function because stuff created on the function stack would be trashed (and figuring
                // out which is is too much pain for the moment)
                ...
                hPipeOut = CreateNamedPipe(
                    ...
                    PIPE_ACCESS_DUPLEX,         // read/write access //tex WORKAROUND was PIPE_ACCESS_OUTBOUND, see
                                                // IHExt/PipeClient for issue, GOTCHA: this means a client could stall
                                                // the pipe if they write as IHHook only treats is as out only
                    PIPE_TYPE_MESSAGE |         // message type pipe
                        PIPE_READMODE_MESSAGE | // message-read mode
                        PIPE_WAIT,              // blocking mode
                    PIPE_UNLIMITED_INSTANCES,   // max. instances
                ...
                // GOTCHA: this means it's waiting for first pipe (pipein) to connect before it tries to connect the second
                // (pipeout)
        ...
        // client connections.
        // tex GOTCHA: however with the current implementation of shunting messages through single message queues, in practice
        // it won't really work out with multiple clients
        DWORD WINAPI PipeOutThread(LPVOID lpvParam)
        {
            ...
            while (!doShutDown)
            {
                ...
                while (messageOpt)
                {
                    ...
                    // GOTCHA: DEBUGNOW still a kinda mess, requires the client to fail its write to close its pipe, then the
                    // server to write here to notice lol
                    ...
                    DWORD messageBytes =
                        static_cast<DWORD>(message.size()) + sizeof('\0'); // tex: std string size() does not include a terminator but c_str() does
        ...
        DWORD WINAPI PipeInThread(LPVOID lpvParam)
        {
            ...
            while (!doShutDown)
            {
                ...
                // Read client requests from the pipe. This simplistic code only allows messages up to BUFSIZE characters in
                // length.
                fSuccess = ReadFile(
                    hPipeIn,                // handle to pipe
                    pchRequest,             // buffer to receive data
