...
namespace IHHook
{
    ...
    // proxy) see Initialize for stuff after
        ...
        : thisModule{ *reinterpret_cast<HMODULE*>(__readgsqword(0x60) + 0x10) }
    {
        ...
        // if (MH_CreateHook(&SetUnhandledExceptionFilter, &UnhandledExceptionFilter_Hook,
        ...
        { // tex GOTCHA: since this continues ih_log will be created thus ./mod will actually exist. so check modules
          // instead
    ...
    void IHH::SetupLog()
    {
        ...
        log->debug(
            "Note: ihhook_log is multithreaded to accept logging from multiple threads so order of entries may not "
            "be sequential.");
        log->flush();
    } // SetupLog
    ...
    bool IHH::OnMessage(HWND wnd, UINT message, WPARAM w_param, LPARAM l_param)
    {
        ...
        if (handledMessage)
        {
            // tex DEBUGNOW WORKAROUND: having menu eat all game can cause a problem if user was holding a key at the time
            // as the keyup even will be eaten
            if (w_param == WM_KEYUP)
            {
                return true;
            }
        ...
        for (auto const& entry : addressSet)
        {
            ...
            else
            {
                ...
                if (it != mgsvtpp_patterns.end())
                {
                    ...
                    {
                        log->debug("sigscan found {} at 0x{:x} in(microseconds): {}", name, addr,
