...
static const char* txtToken(LexState* ls, int token)
{
    ...
    case TK_NAME:
    case TK_STRING:
    case TK_NUMBER: save(ls, '\0'); return luaZ_buffer(ls->buff);
...
static void read_long_string(LexState* ls, SemInfo* seminfo, int sep)
{
    ...
    for (;;)
    {
        switch (ls->current)
        {
        case EOZ: luaX_lexerror(ls, (seminfo) ? "unfinished long string" : "unfinished long comment", TK_EOS); break; /* to avoid warnings */
...
static void read_string(LexState* ls, int del, SemInfo* seminfo)
{
    ...
    while (ls->current != del)
    {
        switch (ls->current)
        {
        case EOZ: luaX_lexerror(ls, "unfinished string", TK_EOS); continue; /* to avoid warnings */
        case '\n':
        case '\r': luaX_lexerror(ls, "unfinished string", TK_STRING); continue; /* to avoid warnings */
        case '\\':
        {
            int c;
