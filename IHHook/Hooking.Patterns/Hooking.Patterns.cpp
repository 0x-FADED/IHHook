            ...
            , m_end(0)
        {
            static auto getSection = [](const PIMAGE_NT_HEADERS nt_headers, WORD section) -> PIMAGE_SECTION_HEADER
            {
                return reinterpret_cast<PIMAGE_SECTION_HEADER>(
                    (UCHAR*)nt_headers->OptionalHeader.DataDirectory + nt_headers->OptionalHeader.NumberOfRvaAndSizes * sizeof(IMAGE_DATA_DIRECTORY)
    ...
    namespace details
    {
        ...
        void basic_pattern_impl::Initialize(std::string_view pattern)
        {
...
#endif
            {
                ...
                {
                    std::for_each(range.first, range.second, [&](const auto& hint) { ConsiderHint(hint.second); });
        ...
        void basic_pattern_impl::EnsureMatches(uint32_t maxCount)
        {
            ...
            __try
            {
                for (uintptr_t i = executable.begin(), end = executable.end() - maskSize; i <= end;)
                {
                    ...
                    while ((j >= 0) && pattern[j] == (ptr[j] & mask[j]))
                        j--;
