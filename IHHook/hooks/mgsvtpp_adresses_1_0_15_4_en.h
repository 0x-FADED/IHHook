...
namespace IHHook
{
    std::unordered_map<std::string, uint64_t> mgsvtpp_adresses_1_0_15_4_en{
        ...
        //{"foxBlockLoad", 0x143151e80},//int * fox::Block::Load(void *thisPtr,int *errorCode,ulonglong *pathID,uint
        // param_4)
        ...
        { "lua_newstate", 0x141a16510 }, // tex could use default implementation, but may want to hook if we want to see what
                                         // the engine is up to
        ...
        //{"lua_isuserdata", USING_CODE},//tex: No calls in lua distro, so may be hard to find, or have been culled by
        // compilation
        { "lua_type", 0x141a12300 },
        //{"lua_typename", USING_CODE},
        //{"lua_equal", NOT_FOUND},//tex: lua implementation goes a bit deeper than I'm happy with to use at the moment. No
        // calls in lua distro, so may be hard to find, or have been culled by compilation
        ...
        //{"lua_yield", USING_CODE},//tex: DEBUGNOW uses lua_lock, may not be a good idea due to thread issues and not
        // knowing what the engine is doing to the state. Seems to be inlined in luaB_yield (it's only call in lua distro)
        { "lua_resume", 0x141a13320 },
        //{"lua_status", USING_CODE},//tex DEBUGNOW hmm, address range. ida finds this as sig though, but the prior
        // functions have entries in .pdata which put them in the same range (0x14cdb)
        { "lua_gc", 0x141a10fb0 },
        { "lua_error", 0x141a10fa0 },
        { "lua_next", 0x141a11600 },
        { "lua_concat", 0x141a10db0 },
        //{"lua_getallocf", NO_USE},//tex don't really want to mess with allocator function anyway, DEBUGNOW no calls in lua
        // distro, so may be hard to find, or have been culled by compilation
        ...
        //{"luaL_optnumber", USING_CODE},//tex: Only use in os_difftime, but decompilation is giving a bunch more params
        ...
        //{"luaL_ref", USING_CODE},//tex: Unsure on this address. No uses in lua dist, found a function that looks much like
        // it, but it was undefined, and has a errant param
        ...
        //{"luaL_addstring", USING_CODE},//tex: Only call is in luaL_gsub, seems to have been optimized out as the function
        // just wraps luaL_addlstring
