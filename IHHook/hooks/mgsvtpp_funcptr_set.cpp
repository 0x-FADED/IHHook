...
// addresses in addressset should be rebased or found by sig scan before this is
// called
//  NOT_FOUND - default for a lapi we want to use, and should actually have
//  found the address in prior exes, but aren't in the current exported address
//  list NO_USE - something we dont really want to use for whatever reason
//  USING_CODE - using the default lapi code implementation instead of hooking

#include "mgsvtpp_func_typedefs.h"

extern ankerl::unordered_dense::map<std::string, uint64_t> addressSet;
...
void SetFuncPtrs()
{
    ...
    // lua_isuserdata =
    // (lua_isuserdataFunc*)addressSet["lua_isuserdata"];//USING_CODE
    ...
    // lua_gethookmask =
    // (lua_gethookmaskFunc*)addressSet["lua_gethookmask"];//USING_CODE
    // lua_gethookcount =
    // (lua_gethookcountFunc*)addressSet["lua_gethookcount"];//USING_CODE
    luaI_openlib = (luaI_openlibFunc*) addressSet["luaI_openlib"];
    // luaL_register =
    // (luaL_registerFunc*)addressSet["luaL_register"];//USING_CODE
    ...
    // luaL_optnumber =
    // (luaL_optnumberFunc*)addressSet["luaL_optnumber"];//USING_CODE
    ...
    // luaL_loadstring =
    // (luaL_loadstringFunc*)addressSet["luaL_loadstring"];//USING_CODE
    ...
    // luaL_buffinit =
    // (luaL_buffinitFunc*)addressSet["luaL_buffinit"];//USING_CODE
    ...
    // luaL_addstring =
    // (luaL_addstringFunc*)addressSet["luaL_addstring"];//USING_CODE
