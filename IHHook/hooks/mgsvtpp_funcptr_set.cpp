...
//  NOT_FOUND - default for a lapi we want to use, and should actually have found the address in prior exes, but aren't
//  in the current exported address list NO_USE - something we dont really want to use for whatever reason USING_CODE -
//  using the default lapi code implementation instead of hooking
...
namespace IHHook
{
    void SetFuncPtrs()
    {
        ff_stringid_hash_n = (ff_stringid_hash_nFunc*)addressSet["ff_stringid_hash_n"];
        FoxStrHash32 = (FoxStrHash32Func*)addressSet["FoxStrHash32"];
        path_hash_code = (path_hash_codeFunc*)addressSet["path_hash_code"];
        FNVHash32 = (FNVHash32Func*)addressSet["FNVHash32"];
        GetFreeMissionNameKey = (GetFreeMissionNameKeyFunc*)addressSet["GetFreeMissionNameKey"];
        UpdateCamera = (UpdateCameraFunc*)addressSet["UpdateCamera"];
        foxprintf = (foxprintfFunc*)addressSet["foxprintf"];
        l_StubbedOut = (l_StubbedOutFunc*)addressSet["l_StubbedOut"];
        voidreturn = (voidreturnFunc*)addressSet["voidreturn"];
        foxPathPath = (foxPathPathFunc*)addressSet["foxPathPath"];
        UpdateLocalPathString = (UpdateLocalPathStringFunc*)addressSet["UpdateLocalPathString"];
        foxPathPathB = (foxPathPathBFunc*)addressSet["foxPathPathB"];
        foxPathEmpty = (foxPathEmptyFunc*)addressSet["foxPathEmpty"];
        GetPartsFpkPath = (GetPartsFpkPathFunc*)addressSet["GetPartsFpkPath"];
        GetPartsFilePath = (GetPartsFilePathFunc*)addressSet["GetPartsFilePath"];
        GetCamoFpkPath = (GetCamoFpkPathFunc*)addressSet["GetCamoFpkPath"];
        GetCamoFilePath = (GetCamoFilePathFunc*)addressSet["GetCamoFilePath"];
        GetFacialMtarFpkPath = (GetFacialMtarFpkPathFunc*)addressSet["GetFacialMtarFpkPath"];
        GetFacialMtarFilePath = (GetFacialMtarFilePathFunc*)addressSet["GetFacialMtarFilePath"];
        GetHandFpkPath = (GetHandFpkPathFunc*)addressSet["GetHandFpkPath"];
        GetHandFilePath = (GetHandFilePathFunc*)addressSet["GetHandFilePath"];
        _DoesNeedBodyFovaForDD = (_DoesNeedBodyFovaForDDFunc*)addressSet["_DoesNeedBodyFovaForDD"];
        GetBodyFovaPath = (GetBodyFovaPathFunc*)addressSet["GetBodyFovaPath"];
        DoesNeedFaceFova = (DoesNeedFaceFovaFunc*)addressSet["DoesNeedFaceFova"];
        DoesNeedFaceFovaForAvatar = (DoesNeedFaceFovaForAvatarFunc*)addressSet["DoesNeedFaceFovaForAvatar"];
        GetFaceFpkPath = (GetFaceFpkPathFunc*)addressSet["GetFaceFpkPath"];
        GetFaceFilePath = (GetFaceFilePathFunc*)addressSet["GetFaceFilePath"];
        GetAvatarHoneFpkPath = (GetAvatarHoneFpkPathFunc*)addressSet["GetAvatarHoneFpkPath"];
        GetAvatarHoneFilePath = (GetAvatarHoneFilePathFunc*)addressSet["GetAvatarHoneFilePath"];
        GetPathAtBuddyType = (GetPathAtBuddyTypeFunc*)addressSet["GetPathAtBuddyType"];
        GetBuddyQuietWeaponFileFromBuddyBlock = (GetBuddyQuietWeaponFileFromBuddyBlockFunc*)addressSet["GetBuddyQuietWeaponFileFromBuddyBlock"];
        GetPathAtSub_BuddyDog = (GetPathAtSub_BuddyDogFunc*)addressSet["GetPathAtSub_BuddyDog"];
        GetPathAtSub_BuddyHorse = (GetPathAtSub_BuddyHorseFunc*)addressSet["GetPathAtSub_BuddyHorse"];
        GetBuddyGearArmFile = (GetBuddyGearArmFileFunc*)addressSet["GetBuddyGearArmFile"];
        GetBuddyGearHeadFile = (GetBuddyGearHeadFileFunc*)addressSet["GetBuddyGearHeadFile"];
        GetBuddyGearMainWeaponFile = (GetBuddyGearMainWeaponFileFunc*)addressSet["GetBuddyGearMainWeaponFile"];
        FoxBlockLoad = (FoxBlockLoadFunc*)addressSet["FoxBlockLoad"];
        ReliefVehicleLoadToExhibit = (ReliefVehicleLoadToExhibitFunc*)addressSet["ReliefVehicleLoadToExhibit"];
        ReliefVehicleLoad = (ReliefVehicleLoadFunc*)addressSet["ReliefVehicleLoad"];
        GetBlockAtIndex = (GetBlockAtIndexFunc*)addressSet["GetBlockAtIndex"];
        gkGetColoringSystem = (gkGetColoringSystemFunc*)addressSet["gkGetColoringSystem"];
        GetBuddyCommandName = (GetBuddyCommandNameFunc*)addressSet["GetBuddyCommandName"];
        GetBuddyCommandHelpName = (GetBuddyCommandHelpNameFunc*)addressSet["GetBuddyCommandHelpName"];
