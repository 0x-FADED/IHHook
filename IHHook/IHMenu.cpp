...
// tex would like to keep it const char* all the way through from lua to imgui
// instead of back and forthing bewtween char* and string, but imgui shits the
// bed at some point when I try that try converting just menuItems to see
...
std::vector<std::string> menuItems{
    "1:Menu line test = 1:SomeSetting", "2:Menu line test = 1:SomeSetting", "3:Menu line test = 1:SomeSetting", "4:Menu line test = 1:SomeSetting", "5:Menu line test = 1:SomeSetting",
};
...
std::string menuHelp = "Some super long textand stuff that might describeand option.Yet more text "
                       "letst see how this wraps.Some super long textand stuff that might "
                       "describeand option.Yet more text letst see how this wraps.And more.Some "
                       "super long textand stuff that might describeand option.Yet more text "
                       "letst see how this wraps.How much more.Some super long textand stuff that "
                       "might describeand option.Yet more text lets see how this wraps.So much "
                       "more.Some super long textand stuff that might describeand option.Yet more "
                       "text letst see how this wraps.";
...
bool TextInputComboBox(const char* id, char* buffer, size_t maxInputSize, const std::vector<std::string>& items, short showMaxItems);

// IH/Lua > IHMenu
// DEBUGNOW tex: this is pretty trash, it's really just getting the IHExt api
// (which was also trash but atleas more flexible so since pushing through WPF)
// satisfied with the least fuss. If IHHook and D3D hook turns out to be robust
// enough for users then IHExt can be ditched and this should be overhauled
// along with the IH side where it pushes menu or imgui specific stuff and
// leaves ExtCmd/Pipe stuff for other purposes

void SetContent(const std::vector<std::string>& args)
{
...
void SetText(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
...
void SetTextBox(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
...
void UiElementVisible(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
    ...
    else if (name == "menuTitle")
    {
        if (visible == 1)
        {
            ...
            // DEBUGNOW g_ihhook->SetCursor(true);//tex now handled by
...
void ClearTable(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 1)
    {
...
void AddToTable(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
...
void UpdateTable(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 3)
    {
...
void SelectItem(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
...
void ClearCombo(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 1)
    {
...
void AddToCombo(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
...
void SelectCombo(const std::vector<std::string>& args)
{
    // spdlog::trace(__func__);
    if (args.size() < 1 + 2)
    {
    ...
    if (name == "menuSetting")
    {
        if (selectedIndex >= 0 && selectedIndex < menuSettings.size())
        {
            ...
            strcpy_s(settingInputBuffer, bufferSize,
                     menuSettings[selectedSetting].c_str()); // DEBUGNOW
        }
        else
        {
...
void ToggleStyleEditor(const std::vector<std::string>& args) { g_ihhook->ToggleStyleEditor(); } // ToggleStyleEditor

void ToggleImguiDemo(const std::vector<std::string>& args) { g_ihhook->ToggleImguiDemo(); } // ToggleImguiDemo

void EnableCursor(const std::vector<std::string>& args) { g_ihhook->SetCursor(true); } // EnableCursor

typedef void (*MenuCommandFunc)(const std::vector<std::string>& args);
...
// DEBUGNOW
void MenuMessage(const std::string& message)
{
    // spdlog::trace(__func__);
...
// IHMenu > IH/Lua
// tex: needs to be thread safe since game/lua is different thread than IHMenu
// (which is on d3d present) DEBUGNOW TODO rename to menuMessagesIn to
// differentiate from pipe?
SafeQueue<std::string> messagesIn;
SafeQueue<std::string> messagesOut;
// tex lua > ihmenu (via l_MenuMessage)
void QueueMessageOut(const std::string& message) { messagesOut.push(message); } // QueueMessageOut
...
void DrawMenu(bool* p_open, bool openPrev)
{
    ...
    // tex: GOTCHA name acts as id by default so setting it to something dynamic
    // like menuTitle means each submenu is a new window so it will have
    // individual position and size if user changes it. Alternative is to
    // menuTitle + "##menuTitle"? or pushID, popID if (!
    ImGui::Begin(windowTitle.c_str(), p_open,
                 windowFlags); // tex: TODO: there's probably a better way to handle the
                               // x/close button somehow rather than this which just
                               // flips a bool QueueMessageIn("togglemenu|1");
    //}
    ...
    //	ImGui::GetForegroundDrawList()->AddRect(vMin, vMax, IM_COL32(255, 255,
    // 0, 255));
    //}
    int helpHeightInItems = 4;
    if (menuHelp == "")
    {                          // tex: 'turning off help' simply sets an empty string
        helpHeightInItems = 1; // tex leave a lines worth of buffer otherwise the window drag
                               // corner icon clashes visually with the combo box
    }
    ...
    //	if (ImGui::InputText("##menuSettingInput", settingInputBuffer,
    // IM_ARRAYSIZE(settingInputBuffer), settingInputFlags)) { 		if
    //(menuSettings.size() == 1) { 			menuSettings[0] =
    // settingInputBuffer;
    // QueueMessageIn("input|menuSetting|" + menuSettings[0]);
    //		}
    //	}
    //} else {//tex use combo box
    //	const char* comboLabel = "";// Label to preview before opening the combo
    //(technically it could be anything) 	if (menuSettings.size() > 0 &&
    // selectedSetting < menuSettings.size()) { 		comboLabel =
    // menuSettings[selectedSetting].c_str();
    //	}
    ...
    //			if (ImGui::Selectable(menuSettings[i].c_str(),
    // selected)) { 				selectedSetting = i;
    //				QueueMessageIn("selectedcombo|menuSetting|" +
    // std::to_string(selectedSetting));
    //			}

    //			// Set the initial focus when opening the combo
    //(scrolling + keyboard navigation focus) 			if (selected) {
    ...
    // DEBUGNOW
    int maxItemsShown = 7; // 0 == show all, but you don't get a scroll bar so it's unusable and
                           // will be off screen for large lists anyhoo
    if (TextInputComboBox("##menuSettings", settingInputBuffer, bufferSize, menuSettings, maxItemsShown))
    {
        ...
        else
        {
            ...
            for (int i = 0; i < menuSettings.size(); i++)
            {
                ...
                { // tex settingInputBuffer matches a setting
                    selectedSetting = i;
                    didSelect = true;
    ...
    ImGui::TextWrapped("%s",
                       menuHelp.c_str()); // tex WORKAROUND: Text widget takes fmted text,
                                          // so slap it in like this so it doesn't choke on
                                          // stuff like %, there's also ::TextUnformatted
                                          // that's more performant, but it doesn't wrap.
    ImGui::EndChild();

    ImGui::End();

    // ImGui::SetNextWindowSizeConstraints(ImVec2(0, -1), ImVec2(0,
    // -1));//DEBUGNOW
} // DrawMenu
...
void DrawList(float contentHeight, int helpHeightInItems)
{
    ...
    // listboxHeightInItems = std::min(listboxHeightInItems,
    // (int)menuItems.size());//tex still deciding whether size menu to its
    // number of items (this line uncommented), or to fill menu to window (this
    // line commented out), and what to do with the bottom of the window
    if (listboxHeightInItems > 0)
    {
        if (ImGui::BeginListBox("##menuItems", ImVec2(0, listboxHeightInItems * ImGui::GetTextLineHeightWithSpacing())))
        {
            for (int i = 0; i < menuItems.size(); i++)
            {
                ImGui::PushItemWidth(-1); // tex push out label
                ImGui::PushID(i);         // tex in theory shouldnt be a problem as
                                          // menu items have a number prefixed
                bool selected = (selectedItem == i);
                // tex putting this before -v- means that ih/lua can set
                // selectedItem (call SelectItem on keyboard scroll) and have
                // this scroll the selection into the view, but let
                // ImGui::Selectable //DEBUGNOW unless there's a loop with
                // togamecmd 'selected' that calls SelectItem again?
                ...
                {
                    selectedItem = i;
                    prevSelectedItem = selectedItem; // tex to stop autoscroll from kicking off
                                                     // since we changed selectedItem
                ...
                // tex set selected as focus otherwise if inputtext has focus on
                // menu open it's annoying if (*p_open && *p_open != openPrev) {
                // DEBUGNOW ImGui::SetItemDefaultFocus(); DEBUGNOW
...
{
    size_t buf_size = std::char_traits<char>::length(buf);
    size_t item_size = std::char_traits<char>::length(item);
    // Check if the item length is shorter or equal --> exclude
    if (buf_size >= item_size)
        return false;
    for (int i = 0; i < std::char_traits<char>::length(buf); ++i)
...
int propose(ImGuiInputTextCallbackData* data)
{
    ...
    if (std::char_traits<char>::length(data->Buf) == 0)
        return 0;

    // Get our items back
    std::vector<std::string>* items = static_cast<std::vector<std::string>*>(data->UserData);
    // WORKAROUND: tex setting is a direct value, don't autocomplete cause its
    // annoying
    ...
    {
        // We delete the last char automatically, since it is what the user
        // wants to delete, but only if there is something
        // (selected/marked/hovered)
...
// bool TextInputComboBox(const char* id, std::string& str, size_t maxInputSize,
// std::vector<std::string> items, short maxItemsShown) { 	if (str.size() >
// maxInputSize) { // too large for editing ImGui::Text(str.c_str());
// return false;
//	}

//	std::string buffer(str);
//	buffer.resize(maxInputSize);
//	bool changed = TextInputComboBox(id, &buffer[0], maxInputSize, items,
// maxItemsShown);
...
// maxItemsShown determines how many items are shown, when the dropdown is open;
// if 0 is passed the complete list will be shown; you will want normaly a value
// of 8 tex adapted from https://github.com/ocornut/imgui/issues/2057 to be
// kinda ihmenu specific
bool TextInputComboBox(const char* id, char* buffer, size_t maxInputSize, const std::vector<std::string>& items, short showMaxItems)
{
    ...
    ImGui::PushID(id);
    // std::pair<const char**, size_t> pass(items, item_len); //We need to pass
    // the array length as well//DEBUGNOW
    ImGui::PushItemWidth(-ImGui::GetFrameHeight()); // tex decrease size by
                                                    // Arrow button default size
    ImGuiInputTextFlags inputFlags = 0;
    inputFlags |= ImGuiInputTextFlags_EnterReturnsTrue;
    inputFlags |= ImGuiInputTextFlags_CallbackAlways;
    bool ret = ImGui::InputText("##in", buffer, maxInputSize, inputFlags, propose, std::bit_cast<void*>(&items));
    if (ret)
    { // DEBUGNOW
        bool blurg = true;
    }
    ...
    if (items.size() > 0 && selectedSetting < items.size())
    {
        ...
        // items[selectedSetting].c_str());//DEBUGNOW
    }
    ...
    // tex TODO: if bottom of popup below bottom of screen then have popup above
    // input/selected line like vanilla comboboxes behaviour.
    ImGuiIO& io = ImGui::GetIO();
    ...
    if (ImGui::BeginPopup("combobox", ImGuiWindowFlags_::ImGuiWindowFlags_NoMove))
    {
        ...
        for (int i = 0; i < items.size(); i++)
        {
            ImGui::PushID(i); // tex in theory shouldnt be a problem as menu
                              // items have a number prefixed
            bool selected = (selectedSetting == i);
