...
namespace IHHook
{
    namespace IHMenu
    {
        ...
        // tex would like to keep it const char* all the way through from lua to imgui instead of back and forthing bewtween
        // char* and string, but imgui shits the bed at some point when I try that try converting just menuItems to see
        ...
        std::vector<std::string> menuItems{
            "1:Menu line test = 1:SomeSetting", "2:Menu line test = 1:SomeSetting", "3:Menu line test = 1:SomeSetting",
            "4:Menu line test = 1:SomeSetting", "5:Menu line test = 1:SomeSetting",
        };
        ...
        std::string menuHelp = "Some super long textand stuff that might describeand option.Yet more text letst see how this wraps.Some super "
                               "long textand stuff that might describeand option.Yet more text letst see how this wraps.And more.Some super long "
                               "textand stuff that might describeand option.Yet more text letst see how this wraps.How much more.Some super long "
                               "textand stuff that might describeand option.Yet more text lets see how this wraps.So much more.Some super long "
                               "textand stuff that might describeand option.Yet more text letst see how this wraps.";
        ...
        // IH/Lua > IHMenu
        // DEBUGNOW tex: this is pretty trash, it's really just getting the IHExt api (which was also trash but atleas more
        // flexible so since pushing through WPF) satisfied with the least fuss. If IHHook and D3D hook turns out to be robust
        // enough for users then IHExt can be ditched and this should be overhauled along with the IH side where it pushes menu
        // or imgui specific stuff and leaves ExtCmd/Pipe stuff for other purposes
        ...
        void DrawMenu(bool* p_open, bool openPrev)
        {
            ...
            // tex: GOTCHA name acts as id by default so setting it to something dynamic like menuTitle means each submenu is a
            // new window so it will have individual position and size if user changes it. Alternative is to menuTitle +
            // "##menuTitle"? or pushID, popID if (!
            ImGui::Begin(
                windowTitle.c_str(),
                p_open,
                windowFlags); // tex: TODO: there's probably a better way to handle the x/close button somehow rather
                              // than this which just flips a bool
            ...
            {                          // tex: 'turning off help' simply sets an empty string
                helpHeightInItems = 1; // tex leave a lines worth of buffer otherwise the window drag corner icon clashes
                                       // visually with the combo box
            }
            ...
            //	if (ImGui::InputText("##menuSettingInput", settingInputBuffer, IM_ARRAYSIZE(settingInputBuffer),
            // settingInputFlags)) { 		if (menuSettings.size() == 1) { 			menuSettings[0] = settingInputBuffer;
            ...
            // DEBUGNOW
            size_t maxItemsShown = 7; // 0 == show all, but you don't get a scroll bar so it's unusable and will be off screen
                                      // for large lists anyhoo
            ...
            ImGui::TextWrapped("%s", menuHelp.c_str()); // tex WORKAROUND: Text widget takes fmted text, so slap it in like this
                                                        // so it doesn't choke on stuff like %, there's also ::TextUnformatted
        ...
        void DrawList(float contentHeight, int helpHeightInItems)
        {
            ...
            // listboxHeightInItems = std::min(listboxHeightInItems, (int)menuItems.size());//tex still deciding whether size
            // menu to its number of items (this line uncommented), or to fill menu to window (this line commented out), and
            // what to do with the bottom of the window
            if (listboxHeightInItems > 0)
            {
                if (ImGui::ListBoxHeader("##menuItems", (int)menuItems.size(), listboxHeightInItems))
                {
                    for (int i = 0; i < menuItems.size(); i++)
                    {
                        ...
                        // tex putting this before -v- means that ih/lua can set selectedItem (call SelectItem on keyboard
                        // scroll) and have this scroll the selection into the view, but let ImGui::Selectable  //DEBUGNOW
                        // unless there's a loop with togamecmd 'selected' that calls SelectItem again?
        ...
        int propose(ImGuiInputTextCallbackData* data)
        {
            ...
            {
                // We delete the last char automatically, since it is what the user wants to delete, but only if there is
                // something (selected/marked/hovered)
        ...
        // bool TextInputComboBox(const char* id, std::string& str, size_t maxInputSize, std::vector<std::string> items, short
        // maxItemsShown) { 	if (str.size() > maxInputSize) { // too large for editing 		ImGui::Text(str.c_str()); 		return false;
        //	}
        ...
        // maxItemsShown determines how many items are shown, when the dropdown is open; if 0 is passed the complete list will
        // be shown; you will want normaly a value of 8 tex adapted from https://github.com/ocornut/imgui/issues/2057 to be
        ...
        bool TextInputComboBox(const char* id, char* buffer, size_t maxInputSize, std::vector<std::string> items, size_t showMaxItems)
        {
            ...
            // tex TODO: if bottom of popup below bottom of screen then have popup above input/selected line like vanilla
            // comboboxes behaviour.
            ImGuiIO& io = ImGui::GetIO();
