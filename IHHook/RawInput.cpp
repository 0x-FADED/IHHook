...
// By intercepting this we can not only give IHHook a method of input but could
// also selectively block input to the game.

// DEBUGNOW this only really gets you OnKeyDown, OnKeyUp reliably as Held will
// be limited by key repeat rate the solution there would be to have another
// state array and have the input events set up,down and querry that with the
// assumption that down is held
...
plf::list<ButtonAction>* buttonActions[vKeyMax] = {NULL};

void BlockMouseClick() { blockGameKeys[VK_LBUTTON] = true; } // BlockMouseClick

void UnBlockMouseClick() { blockGameKeys[VK_LBUTTON] = false; } // UnBlockMouseClick
...
void ProcessKey(PRAWINPUT pRaw)
{
    ...
    // Create a combined state value
    int state = (flags << 16) | oldFlags;

    switch (state)
    {
    case (RI_KEY_MAKE << 16) | RI_KEY_BREAK: buttonEvent = BUTTONEVENT::ONDOWN; break;
    case (RI_KEY_BREAK << 16) | RI_KEY_MAKE: buttonEvent = BUTTONEVENT::ONUP; break;
    case (RI_KEY_MAKE << 16) | RI_KEY_MAKE: buttonEvent = BUTTONEVENT::HELD; break;
    default: buttonEvent = BUTTONEVENT::UP; break;
    }
...
struct
{
    ...
    UINT downflag;
    UINT upflag;
} const k[] = {{VK_LBUTTON, RI_MOUSE_LEFT_BUTTON_DOWN, RI_MOUSE_LEFT_BUTTON_UP}, {VK_RBUTTON, RI_MOUSE_RIGHT_BUTTON_DOWN, RI_MOUSE_RIGHT_BUTTON_UP}, {VK_MBUTTON, RI_MOUSE_MIDDLE_BUTTON_DOWN, RI_MOUSE_MIDDLE_BUTTON_UP}, {VK_XBUTTON1, RI_MOUSE_BUTTON_4_DOWN, RI_MOUSE_BUTTON_4_UP}, {VK_XBUTTON2, RI_MOUSE_BUTTON_5_DOWN, RI_MOUSE_BUTTON_4_UP}};
...
void DoActions(USHORT vKey, RawInput::BUTTONEVENT buttonEvent)
{
    plf::list<ButtonAction>* actions = buttonActions[vKey];
    if (actions != NULL)
    {
        spdlog::debug("RawInput DoActions for vKey:{}", vKey);
        for (plf::list<ButtonAction>::iterator it = actions->begin(); it != actions->end(); ++it)
        {
...
void RegisterAction(USHORT vKey, ButtonAction action)
{
    ...
    {
        buttonActions[vKey] = new plf::list<ButtonAction>();
    }
...
{
    spdlog::debug("ButtonEvent: {:d}, Action: TestAction", static_cast<int32_t>(buttonEvent));

    switch (buttonEvent)
    {
    case BUTTONEVENT::ONDOWN: spdlog::debug("TestAction on ONDOWN"); break;

    case BUTTONEVENT::ONUP: spdlog::debug("TestAction on ONUP"); break;

    case BUTTONEVENT::HELD: spdlog::debug("TestAction on HELD"); break;

    default: spdlog::debug("TestAction on UNKNOWN button event"); break;
    }
} // TestAction

void ToggleUI(RawInput::BUTTONEVENT buttonEvent)
{
    spdlog::debug("ButtonEvent: {:d}, Action: ToggleUI", static_cast<int32_t>(buttonEvent));
...
{
    spdlog::debug("ButtonEvent: {:d}, Action: ToggleCursor", static_cast<int32_t>(buttonEvent));

    switch (buttonEvent)
    {
    case RawInput::BUTTONEVENT::ONDOWN:
        spdlog::debug("ToggleCursor on ONDOWN");
        g_ihhook->ToggleCursor();
        break;

    case RawInput::BUTTONEVENT::ONUP: spdlog::debug("ToggleCursor on ONUP"); break;

    case RawInput::BUTTONEVENT::HELD: spdlog::debug("ToggleCursor on HELD"); break;

    default: spdlog::debug("ToggleCursor on UNKNOWN ToggleCursor event"); break;
    }
}
// ToggleCursor
...
{
    spdlog::debug("ButtonEvent: {:d}, Action: ToggleImguiDemo", static_cast<int32_t>(buttonEvent));
    if (buttonEvent == RawInput::BUTTONEVENT::ONDOWN)
    {
...
{
    spdlog::debug("ButtonEvent: {:d}, Action: ToggleStyleEditor", static_cast<int32_t>(buttonEvent));
    if (buttonEvent == RawInput::BUTTONEVENT::ONDOWN)
    {
...
// DEBUGNOW
// tex GOTCHA: WORKAROUND: The game stops lua updates (all gameplay updates I
// guess) in the pause menu, this didn't matter much when IH was lua only,
// because it would catch that ESC was pressed when the engine resumed the lua
// state however since IMGUI is run on present hook/a different thread the delay
// can put things in a bad state so just setting SetDrawUI(false) and menuoff
// will just run whenever DEBUGNOW rename, this is menuoff
...
void InitializeRawInputDevices()
{
    ...
    Rid[0].usUsage = 0x02;
    // Rid[0].dwFlags = RIDEV_NOLEGACY;   // adds HID mouse and also ignores
    // legacy mouse messages
    Rid[0].hwndTarget = 0;

    Rid[1].usUsagePage = 0x01;
    Rid[1].usUsage = 0x06;
    // Rid[1].dwFlags = RIDEV_NOLEGACY;   // adds HID keyboard and also ignores
...
bool OnMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INPUT:
    {
        // wParam is either RIM_INPUT (this app foreground) or RIM_INPUTSINK
