...
// By intercepting this we can not only give IHHook a method of input but could also selectively block input to the
// game.

// DEBUGNOW this only really gets you OnKeyDown, OnKeyUp reliably as Held will be limited by key repeat rate
// the solution there would be to have another state array and have the input events set up,down and querry that with
// the assumption that down is held
...
namespace IHHook
{
    namespace RawInput
    {
        ...
        struct
        {
            ...
            UINT downflag;
            UINT upflag;
        } const k[] = { { VK_LBUTTON, RI_MOUSE_LEFT_BUTTON_DOWN, RI_MOUSE_LEFT_BUTTON_UP },
        ...
        // this didn't matter much when IH was lua only, because it would catch that ESC was pressed when the engine resumed the
        // lua state however since IMGUI is run on present hook/a different thread the delay can put things in a bad state so
        // just setting SetDrawUI(false) and menuoff will just run whenever DEBUGNOW rename, this is menuoff
