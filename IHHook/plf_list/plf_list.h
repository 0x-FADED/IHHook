// Copyright (c) 2024, Matthew Bentley (mattreecebentley@gmail.com)
// www.plflib.org
...
// Suppress incorrect (unfixed MSVC bug) warnings re: constant expressions in
...
#endif
#if _MSC_VER >= 1800
#define PLF_VARIADICS_SUPPORT // Variadics, in this context, means both
                              // variadic templates and variadic macros are
                              // supported
...
#define PLF_INITIALIZER_LIST_SUPPORT
#endif
#elif defined(__GLIBCXX__) // Using another compiler type with libstdc++ - we
                           // are assuming full c++11 compliance for compiler -
...
#define PLF_ALIGNMENT_SUPPORT
#endif
#if __GLIBCXX__ >= 20150422 // libstdc++ v4.9 and below do not support
...
#define PLF_IS_ALWAYS_EQUAL_SUPPORT
#endif
#elif defined(_LIBCPP_CXX03_LANG) || defined(_LIBCPP_HAS_NO_RVALUE_REFERENCES) // Special case for checking
                                                                               // C++11 support with libCPP
#if !defined(_LIBCPP_HAS_NO_VARIADICS)
#define PLF_VARIADICS_SUPPORT
#endif
#else // Assume type traits and initializer support for other compilers and
...
#if __cplusplus >= 201703L && ((defined(__clang__) && ((__clang_major__ == 3 && __clang_minor__ == 9) || __clang_major__ > 3)) || (defined(__GNUC__) && __GNUC__ >= 7) || (!defined(__clang__) && !defined(__GNUC__))) // assume correct C++17 implementation for
...
namespace plf
{
// For getting std:: overload for reverse_iterator to match plf::list iterators
// specifically (see bottom of header):
template <class T>
...
// Until such point as standard libraries include std::ranges::from_range_t,
// including this so the rangesv3 constructor overloads will work unambiguously:
namespace ranges
{
struct from_range_t
{
...
template <class element_type> struct less
{
    bool operator()(const element_type& a, const element_type& b) const PLF_NOEXCEPT { return a < b; }
};

template <class element_type> struct equal_to
{
    const element_type& value;

    explicit equal_to(const element_type& store_value) PLF_NOEXCEPT : value(store_value) {}

    bool operator()(const element_type& compare_value) const PLF_NOEXCEPT { return value == compare_value; }
};

// To enable conversion to void * when allocator supplies non-raw pointers:
template <class source_pointer_type> static PLF_CONSTFUNC void* void_cast(const source_pointer_type source_pointer) PLF_NOEXCEPT
{
#ifdef PLF_CPP20_SUPPORT
...
template <class iterator_type> static PLF_CONSTFUNC std::move_iterator<iterator_type> make_move_iterator(iterator_type it) { return std::move_iterator<iterator_type>(std::move(it)); }
#endif
...
template <class element_type, class allocator_type = std::allocator<element_type>> class list : private allocator_type
{
  public:
    ...
    friend class list_iterator<false>; // Using 'iterator' typedef name here is
                                       // illegal under C++03
  ...
  private:
    ...
    struct node_base
    {
        ...
        node_base(const node_pointer_type& n, const node_pointer_type& p) PLF_NOEXCEPT : next(n), previous(p) {}

#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        node_base(node_pointer_type&& n, node_pointer_type&& p) PLF_NOEXCEPT : next(std::move(n)), previous(std::move(p)) {}
#endif
    };
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        node(node_pointer_type&& next, node_pointer_type&& previous, element_type&& source) PLF_NOEXCEPT : node_base(std::move(next), std::move(previous)), element(std::move(source)) {}
#endif
    ...
    struct group : public node_allocator_type // Node memory block + metadata
    {
        ...
        group() PLF_NOEXCEPT : nodes(NULL), free_list_head(NULL), beyond_end(NULL), number_of_elements(0) {}
...
#else
        // This is a hack around the fact that allocator_type::construct only
        // supports copy construction in C++03 and copy elision does not occur
        // on the vast majority of compilers in this circumstance. And to avoid
        // running out of memory (and performance loss) from allocating the same
        // block twice, we're allocating in this constructor and moving data in
        // the copy constructor.
        group(const group_size_type group_size, node_pointer_type const previous = NULL) PLF_NOEXCEPT : nodes(NULL), free_list_head(previous), beyond_end(NULL), number_of_elements(group_size) {}

        // Not a real copy constructor ie. actually a move constructor. Only
        // used for allocator.construct in C++03 for reasons stated above:
        ...
        group& operator=(const group& source) PLF_NOEXCEPT // Actually a move operator, used by c++03 in
                                                           // group_vector's remove, expand_capacity and append
                                                           // functions
        {
            nodes = source.nodes;
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        group(group&& source) PLF_NOEXCEPT : node_allocator_type(source), nodes(std::move(source.nodes)), free_list_head(std::move(source.free_list_head)), beyond_end(std::move(source.beyond_end)), number_of_elements(source.number_of_elements)
        {
            source.nodes = NULL;
        ...
        ~group() PLF_NOEXCEPT { PLF_DEALLOCATE(node_allocator_type, *this, nodes, static_cast<size_type>(beyond_end - nodes)); }
    };

    class group_vector : private allocator_type // Simple vector of groups +
                                                // associated functions
    {
      public:
        group_pointer_type last_endpoint_group, block_pointer,
            last_searched_group; // last_endpoint_group is the last -active-
                                 // group in the block. Other -inactive-
                                 // (previously used, now empty of elements)
                                 // groups may be stored after this group for
                                 // future usage (to reduce
                                 // deallocation/reallocation of nodes).
                                 // block_pointer + size - 1 == the last group
                                 // in the block, regardless of whether or not
                                 // the group is active.
        size_type size;
        ...
        struct ebco_pair2 : node_pointer_allocator_type // empty-base-class optimisation
        {
            ...
            ebco_pair2(const size_type number_of_elements, const allocator_type& alloc) PLF_NOEXCEPT : node_pointer_allocator_type(alloc), capacity(number_of_elements){};
        } node_pointer_allocator_pair;

        struct ebco_pair : group_allocator_type
        {
            size_type capacity; // Total number of groups
            ebco_pair(const size_type number_of_groups, const allocator_type& alloc) PLF_NOEXCEPT : group_allocator_type(alloc), capacity(number_of_groups){};
        } group_allocator_pair;

        group_vector(const allocator_type& alloc) PLF_NOEXCEPT : allocator_type(alloc), last_endpoint_group(NULL), block_pointer(NULL), last_searched_group(NULL), size(0), node_pointer_allocator_pair(0, alloc), group_allocator_pair(0, alloc) {}

        void blank() PLF_NOEXCEPT
        {
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT // allocator_traits and type_traits always
                                   // available when is_always_equal is
                                   // available
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        group_vector(group_vector&& source) PLF_NOEXCEPT : allocator_type(source), last_endpoint_group(std::move(source.last_endpoint_group)), block_pointer(std::move(source.block_pointer)), last_searched_group(std::move(source.last_searched_group)), size(source.size), node_pointer_allocator_pair(source.node_pointer_allocator_pair.capacity, source), group_allocator_pair(source.group_allocator_pair.capacity, source) { source.blank(); }
        ...
        void clear(const node_pointer_type last_endpoint_node) PLF_NOEXCEPT
        {
            for (group_pointer_type current_group = block_pointer; current_group != last_endpoint_group; ++current_group)
            {
...
#endif
                {
                    ...
                    if ((end - current_group->nodes) != current_group->number_of_elements) // If there are erased
...
#endif
            {
                if ((last_endpoint_node - last_endpoint_group->nodes) != last_endpoint_group->number_of_elements) // If there are erased nodes
...
#endif
                        {
                            if (current_node->next != NULL) // is not part of free list ie. element
                                                            // has not already had it's destructor
                                                            // called
                            {
                                PLF_DESTROY(allocator_type, *this, &(current_node->element));
                            }
...
#endif
            {
                // If allocator supplies non-trivial pointers it becomes
                // necessary to destroy the group. uninitialized_copy will not
                // work in this context as the copy constructor for "group" is
                // overriden in C++03/98. The = operator for "group" has been
                // overriden to make the following work:
        ...
        void add_new(const group_size_type group_size)
        {
            ...
            ++last_endpoint_group; // Doing this here instead of pre-construct
                                   // to avoid need for a try-catch block
            node_pointer_allocator_pair.capacity += group_size;
            ++size;
        }

        void initialize(const group_size_type first_group_capacity) // For adding first group *only*
                                                                    // when group vector is completely
                                                                    // empty and block_pointer is NULL
        {
            ...
            size = 1; // Doing these here instead of pre-construct to avoid need
...
#define PLF_TO_ADDRESS(pointer) &*(pointer)
        ...
        group_pointer_type get_nearest_freelist_group(const node_pointer_type location_node) PLF_NOEXCEPT // In working implementation this cannot throw
        {
            ...
            if (location_node >= last_searched_group->nodes && location_node < last_searched_group->beyond_end) // ie. location is within
                                                                                                                // last_search_group
            {
                if (last_searched_group->free_list_head != NULL)
                    return last_searched_group; // if last_searched_group has
                                                // previously-erased nodes
                // Else: search outwards using loop below this if/else block
            }
            else // search for the node group which location_node is located
                 // within, using last_searched_group as a starting point and
                 // searching left and right. Try and find the closest node
                 // group with reusable erased-element locations along the way:
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (location_node < right->beyond_end && location_node >= right->nodes) // location_node's
                                                                                                // group is found
                        {
                            last_searched_group = right;
                            if (right->free_list_head != NULL)
                                return right; // group has erased nodes, reuse
                                              // them:
                            ...
                            if (closest_freelist_right != NULL)
                            {
                                ...
                                if (left_distance <= 2)
                                    return closest_freelist_right; // ie. this
                                                                   // group is
                                                                   // close
                                                                   // enough to
                                                                   // location_node's
                            ...
                            // check an equal distance on the right to the
                            // distance we've checked on the left:
                            ...
                            break; // group with reusable erased nodes not found
                                   // yet, continue searching in loop below
                        }

                        if (right->free_list_head != NULL) // location_node's group not found, but a
                                                           // reusable location found
                        {
            ...
            // The node group which location_node is located within, is known at
            // this point. Continue searching outwards from this group until a
            // group is found with a reusable location:
            while (true)
            {
                if (right_not_beyond_back)
                {
        ...
        {
#ifdef PLF_IS_ALWAYS_EQUAL_SUPPORT
            if PLF_CONSTEXPR (std::allocator_traits<allocator_type>::is_always_equal::value && std::is_trivial<group_pointer_type>::value) // if all pointer types are trivial we
                                                                                                                                           // can just copy using memcpy - avoids
                                                                                                                                           // constructors/destructors etc and is
                                                                                                                                           // faster
            {
...
#endif
            {
                ...
                // as-possible (yes, it is faster than using std::swap with the
        ...
        void trim_unused_groups() PLF_NOEXCEPT // trim trailing groups previously allocated by
                                               // reserve() or retained via erase()
        {
  ...
  private:
    group_vector groups; // Structure which contains all groups (structures
                         // containing node memory blocks + block metadata)
    node_base end_node;  // The independent, content-less node which is returned
                         // by end()
    // When the list is empty, the previous and next pointers of end_node both
    // point to end_node.
    node_pointer_type last_endpoint; // The node location which is one-past the last inserted
                                     // element in the last group of the list. Is not affected
                                     // by erasures to prior elements (these are handled using
                                     // the group's freelist).
    // If last_endpoint is beyond the end of a memory block it means a new group
    // must be created upon the next insertion if prior erased nodes are not
    // available for re-use. last_endpoint == NULL means total_size is zero, but
    // there may still be groups available due to calling clear(), reserve() on
    // an empty list, or having erased all elements in the list
    // groups.block_pointer
    // == NULL means an uninitialized container ie. no groups or elements yet
    iterator end_iterator, begin_iterator; // Returned by begin() and end().
    // end_iterator always points to end_node. It is a convenience/optimization
    // variable to save generating many temporary iterators from end_node during
    // functions and during end(). When the list is empty of elements,
    // begin_iterator == end_iterator so that program loops iterating from
    // begin() to end() will function as expected.
    size_type total_size;

    struct ebco_pair3 : node_allocator_type
    {
        size_type number_of_erased_nodes;
        ebco_pair3(const size_type num_nodes, const allocator_type& alloc) PLF_NOEXCEPT : node_allocator_type(alloc), number_of_erased_nodes(num_nodes){};
    } node_allocator_pair;

  public:
    // Default constructor:

    PLF_CONSTFUNC list() PLF_NOEXCEPT_ALLOCATOR : groups(*this), end_node(static_cast<node_pointer_type>(&end_node), static_cast<node_pointer_type>(&end_node)), last_endpoint(NULL), end_iterator(static_cast<node_pointer_type>(&end_node)), begin_iterator(static_cast<node_pointer_type>(&end_node)), total_size(0), node_allocator_pair(0, *this) {}
  ...
  public:
    ...
    list(list&& source) PLF_NOEXCEPT : allocator_type(std::move(static_cast<allocator_type&>(source))), groups(std::move(source.groups)), end_node(std::move(source.end_node)), last_endpoint(std::move(source.last_endpoint)), end_iterator(static_cast<node_pointer_type>(&end_node)), begin_iterator((source.begin_iterator.node_pointer == source.end_iterator.node_pointer) ? static_cast<node_pointer_type>(&end_node) : std::move(source.begin_iterator)), total_size(source.total_size), node_allocator_pair(source.node_allocator_pair.number_of_erased_nodes, source)
    {
    ...
    ~list() PLF_NOEXCEPT { groups.destroy_all_data(last_endpoint); }
    ...
    const_iterator cbegin() const PLF_NOEXCEPT { return const_iterator(begin_iterator.node_pointer); }

    const_iterator cend() const PLF_NOEXCEPT { return const_iterator(end_iterator.node_pointer); }

    reverse_iterator rbegin() PLF_NOEXCEPT { return reverse_iterator(end_node.previous); }

    const_reverse_iterator rbegin() const PLF_NOEXCEPT { return const_reverse_iterator(end_node.previous); }

    reverse_iterator rend() PLF_NOEXCEPT { return reverse_iterator(end_iterator.node_pointer); }

    const_reverse_iterator rend() const PLF_NOEXCEPT { return const_reverse_iterator(end_iterator.node_pointer); }

    const_reverse_iterator crbegin() const PLF_NOEXCEPT { return const_reverse_iterator(end_node.previous); }

    const_reverse_iterator crend() const PLF_NOEXCEPT { return const_reverse_iterator(end_iterator.node_pointer); }
    ...
    void clear() PLF_NOEXCEPT
    {
        if (last_endpoint == NULL)
            return; // already clear'ed or uninitialized
        if (total_size != 0)
  ...
  private:
    void add_group_if_necessary()
    {
        ...
        {
            if (static_cast<size_type>(groups.last_endpoint_group - groups.block_pointer) == groups.size - 1) // ie. there are no reusable groups
                                                                                                              // available at the back of group vector
            {
    ...
    static PLF_CONSTFUNC group_size_type list_min_block_capacity() PLF_NOEXCEPT { return static_cast<group_size_type>((sizeof(node) * 8 > (sizeof(list) + sizeof(group)) * 2) ? 8 : (((sizeof(list) + sizeof(group)) * 2) / sizeof(node)) + 1); }

    static PLF_CONSTFUNC group_size_type list_max_block_capacity() PLF_NOEXCEPT { return 2048u; }

    void insert_initialize()
    {
        if (groups.block_pointer == NULL)
            groups.initialize(list_min_block_capacity()); // In case of prior reserve/clear
                                                          // call as opposed to being
                                                          // uninitialized
  ...
  public:
    ...
    void push_front(const element_type& element) { insert(begin_iterator, element); }

#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    iterator insert(const const_iterator it,
                    element_type&& element) // This is almost identical to the
                                            // insert implementation above with
                                            // the only change being std::move of
                                            // the element and the is_nothrow test
    {
        if (last_endpoint != NULL)
        {
    ...
    void push_back(element_type&& element) { insert(end_iterator, std::move(element)); }

    void push_front(element_type&& element) { insert(begin_iterator, std::move(element)); }
#endif

#ifdef PLF_VARIADICS_SUPPORT
    template <typename... arguments>
    iterator emplace(const const_iterator it,
                     arguments&&... parameters) // This is almost identical to the insert
                                                // implementations above with the only
                                                // changes being std::forward of element
                                                // parameters, removal of VARIADICS support
                                                // checking, and is_nothrow_contructible
    {
        if (last_endpoint != NULL)
        {
            if (node_allocator_pair.number_of_erased_nodes == 0)
    ...
    template <typename... arguments> reference emplace_back(arguments&&... parameters) { return (emplace(end_iterator, std::forward<arguments>(parameters)...)).node_pointer->element; }

    template <typename... arguments> reference emplace_front(arguments&&... parameters) { return (emplace(begin_iterator, std::forward<arguments>(parameters)...)).node_pointer->element; }
  ...
  private:
    ...
    template <class iterator_type> iterator_type range_fill(iterator_type it, group_size_type number_of_elements, node_pointer_type const position)
    {
        position->previous->next = last_endpoint;
        groups.last_endpoint_group->number_of_elements = static_cast<group_size_type>(groups.last_endpoint_group->number_of_elements + number_of_elements);
        node_pointer_type previous = position->previous;
    ...
    // This function is near-identical to fill-insert, the only difference is it
    // uses and iterates over an iterator:
    template <class iterator_type> iterator range_insert(const const_iterator position, const size_type number_of_elements, iterator_type it)
    {
  ...
  public:
    ...
    template <class iterator_type> iterator insert(const const_iterator position, typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first, const iterator_type last)
    {
        using std::distance;
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    template <class iterator_type> iterator insert(const const_iterator position, const std::move_iterator<iterator_type> first, const std::move_iterator<iterator_type> last)
    {
        using std::distance;
...
#ifdef PLF_INITIALIZER_LIST_SUPPORT
    iterator insert(const const_iterator it, const std::initializer_list<element_type>& element_list) { return range_insert(it, static_cast<size_type>(element_list.size()), element_list.begin()); }
#endif
  ...
  public:
    ...
    iterator erase(const const_iterator it) // if uninitialized/invalid iterator
                                            // supplied, function could generate
                                            // an exception, hence no noexcept
    {
...
#endif
        {
            PLF_DESTROY(allocator_type, *this,
        ...
        // find the group this element is in, starting from the last group an
        // element to-be-erased was found in (as erasures are, for most
        // programs, likely to be closer in proximity to previous erasures):
        ...
        {
            // Search groups to the left and right of the last searched group,
            // in the group vector:
        ...
        {
            it.node_pointer->next = NULL; // next == NULL so that destructor and other functions
                                          // which linearly iterate over node memory chunks can
                                          // detect this as a free list node, ie an erased node
            it.node_pointer->previous = node_group->free_list_head;
            node_group->free_list_head = it.node_pointer;
            return return_iterator;
        }
        else if (node_group != groups.last_endpoint_group--) // remove group (and decrement
                                                             // active back group)
        {
            ...
            if ((group_size == list_max_block_capacity()) | (node_group >= groups.last_endpoint_group - 1)) // Preserve only groups which are at the maximum
                                                                                                            // possible size, or first/second/third-to-last
                                                                                                            // active groups - seems to be best for
                                                                                                            // performance under high-modification
                                                                                                            // benchmarks
            {
        ...
        else // clear back group, leave trailing
        {
            ...
            {
                groups.last_endpoint_group = groups.block_pointer; // If number of elements is zero, it
                                                                   // indicates that this was the only
                                                                   // group in the vector. In which case
                                                                   // the last_endpoint_group would be
                                                                   // invalid at this point due to the
                                                                   // decrement in the above else-if
                                                                   // statement. So it needs to be reset,
                                                                   // as it will not be reset in the
                                                                   // function call below.
                blank();
            }
    ...
    void pop_front() { erase(begin_iterator); }
...
#endif
            { // Deallocate existing blocks as source allocator is not
              // necessarily
                // able to do so
                groups.destroy_all_data(last_endpoint);
            }
...
// Move source values across:
        ...
        else // Allocator isn't movable so move elements from source and
    ...
    friend bool operator!=(const list& lh, const list& rh) PLF_NOEXCEPT { return !(lh == rh); }

#ifdef PLF_CPP20_SUPPORT
    friend constexpr std::strong_ordering operator<=>(const list& lh, const list& rh) { return std::lexicographical_compare_three_way(lh.begin(), lh.end(), rh.begin(), rh.end()); }

    [[nodiscard]]
#endif
    bool
    empty() const PLF_NOEXCEPT
    {
        return total_size == 0;
    }
    ...
    size_type capacity() const PLF_NOEXCEPT { return groups.node_pointer_allocator_pair.capacity; }

    size_type memory() const PLF_NOEXCEPT { return sizeof(*this) + (groups.node_pointer_allocator_pair.capacity * sizeof(node)) + (groups.group_allocator_pair.capacity * sizeof(group)); }

  private:
    // Function-object to redirect the sort function to sort pointers by the
    // elements they point to, not the pointer value
    template <class comparison_function> struct sort_dereferencer
    {
        comparison_function stored_instance;

        explicit sort_dereferencer(const comparison_function& function_instance) PLF_NOEXCEPT : stored_instance(function_instance) {}

        bool operator()(const node_pointer_type first, const node_pointer_type second) { return stored_instance(first->element, second->element); }
    };

  public:
    template <class comparison_function> void sort(comparison_function compare)
    {
        if (total_size < 2)
            return;
        ...
        // According to the C++ standard, construction of a pointer (of any
        // type) may not trigger an exception - hence, no try-catch blocks are
        // necessary for constructing the pointers:
        for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
        {
            const node_pointer_type end = current_group->beyond_end;

            if (end - current_group->nodes != current_group->number_of_elements) // If there are erased nodes
                                                                                 // present in the group
            {
                for (node_pointer_type current_node = current_group->nodes; current_node != end; ++current_node)
                {
        ...
        if (last_endpoint - groups.last_endpoint_group->nodes != groups.last_endpoint_group->number_of_elements) // If there are erased nodes present in
                                                                                                                 // the group
        {
    ...
    void splice(const const_iterator position, const const_iterator first,
                const const_iterator last) PLF_NOEXCEPT // intra-list only splice functions - will crash if first &
                                                        // list are not from *this
    {
        if (position == last)
            return;
        ...
        // To avoid pointer aliasing and subsequently increase performance via
    ...
    void splice(const const_iterator position, const const_iterator location) PLF_NOEXCEPT { splice(position, location, const_iterator(location.node_pointer->next)); }
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    void splice(iterator position, list&& source) { splice(position, source); }
#endif

    template <class comparison_function> void merge(list& source, comparison_function compare)
    {
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    template <class comparison_function> void merge(list&& source, comparison_function compare) { merge(source, compare); }

    void merge(list&& source) { merge(source); }
#endif

    void reverse() PLF_NOEXCEPT
    {
        // Note: current_node->next has to be read during swapping in this
        // process anyway, so including a test to figure out whether or not a
        // given group has erased elements within it, and thus avoid per-node
        // tests, is actually detrimental to performance according to
        // benchmarks. This is unlike sort() where current_node->next is not
        // used in the rest of the process and avoiding per-node tests of it's
        // value is therefore beneficial in benchmarks.
        if (total_size > 1)
        {
            for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
            {
  ...
  public:
    template <class comparison_function> size_type unique(comparison_function compare)
    {
    ...
    size_type unique() { return unique(eq()); }

    template <class predicate_function> size_type remove_if(predicate_function predicate)
    {
        const size_type original_number_of_elements = total_size;
        ...
        if (original_number_of_elements != 0)
        {
            for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
            {
                ...
                if (end - current_group->nodes != num_elements) // If there are erased nodes present in the
                                                                // group
                {
                    for (node_pointer_type current_node = current_group->nodes; current_node != end; ++current_node)
                    {
                        if (current_node->next != NULL && predicate(current_node->element)) // is not free list node
                                                                                            // and validates predicate
                        {
                            erase(current_node);

                            if (--num_elements == 0) // ie. group will be empty (and removed)
                                                     // now - nothing left to iterate over
                            {
                                --current_group; // As current group has been
                                                 // removed, subsequent groups
                                                 // have already shifted back by
                                                 // one, hence, the ++ to the
                                                 // current group in the for
                                                 // loop is unnecessary, and
                                                 // negated here
                                break;
                            }
                        }
            ...
            if (last_endpoint - groups.last_endpoint_group->nodes != num_elements) // If there are erased nodes present in the
                                                                                   // group
            {
                for (node_pointer_type current_node = groups.last_endpoint_group->nodes; current_node != last_endpoint; ++current_node)
                {
    ...
    size_type remove(const element_type& value) { return remove_if(plf::equal_to<element_type>(value)); }
    ...
    void reserve(size_type reserve_amount)
    {
        ...
        { // edge case: has been filled with elements then
          // clear()'d - some groups may be smaller than would
          // be desired, should be replaced
            ...
            if (reserve_amount > end_group_size && end_group_size != list_max_block_capacity()) // if last group isn't
                                                                                                // large enough, remove all
                                                                                                // groups
            {
                reset();
            }
            else
            {
                ...
                // Remove any max_size groups which're not needed and any groups
                // that're smaller than remainder:
        ...
        if (groups.block_pointer == NULL) // Previously uninitialized list or reset in above if
                                          // statement; most common scenario
        {
            if (reserve_amount != 0)
            {
        ...
        { // Create a group at least as large as the last group - may
          // allocate more than necessary, but better solution than
          // creating a very small group in the middle of the group
          // vector, I think:
    ...
    void trim_capacity() PLF_NOEXCEPT { groups.trim_unused_groups(); }
    ...
    void shrink_to_fit()
    {
        ...
        else if (node_allocator_pair.number_of_erased_nodes == 0 && last_endpoint == groups.last_endpoint_group->beyond_end) // edge case - currently no wasted
                                                                                                                             // space except for possible
                                                                                                                             // trailing groups
        {
...
#ifdef PLF_TYPE_TRAITS_SUPPORT
        if PLF_CONSTEXPR (std::is_move_assignable<element_type>::value && std::is_move_constructible<element_type>::value) // move elements if possible,
                                                                                                                           // otherwise copy them
        {
    ...
    template <class iterator_type> void assign(const typename plf::enable_if<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first, const iterator_type last)
    {
        clear();
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    template <class iterator_type> void assign(const const_iterator position, const std::move_iterator<iterator_type> first, const std::move_iterator<iterator_type> last)
    {
        clear();
        using std::distance;
    ...
    allocator_type get_allocator() const PLF_NOEXCEPT { return allocator_type(); }

    template <class predicate_function> iterator unordered_find_single(predicate_function predicate) const PLF_NOEXCEPT
    {
        if (total_size != 0)
        {
            for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
            {
                                                                                     ...
                                                                                     // present in the group
                {
                    ...
                    {
                        if (current_node->next != NULL && predicate(current_node->element))
                            return iterator(current_node); // is not free list node and
                                                           // matches element
                    }
                }
                else // No erased nodes in group
    ...
    iterator unordered_find_single(const element_type& element_to_match) const PLF_NOEXCEPT { return unordered_find_single(plf::equal_to<element_type>(element_to_match)); }

    template <class predicate_function> list<iterator> unordered_find_multiple(predicate_function predicate, size_type number_to_find) const
    {
    ...
    list<iterator> unordered_find_multiple(const element_type& element_to_match, size_type number_to_find) const { return unordered_find_multiple(plf::equal_to<element_type>(element_to_match), number_to_find); }

    template <class predicate_function> list<iterator> unordered_find_all(predicate_function predicate) const
    {
        list<iterator> return_list;
    ...
    list<iterator> unordered_find_all(const element_type& element_to_match) const { return unordered_find_all(plf::equal_to<element_type>(element_to_match)); }
    ...
    template <bool is_const> class list_iterator
    {
      ...
      public:
        ...
        bool operator==(const list_iterator rh) const PLF_NOEXCEPT { return (node_pointer == rh.node_pointer); }

        bool operator==(const list_iterator<!is_const> rh) const PLF_NOEXCEPT { return (node_pointer == rh.node_pointer); }

        bool operator!=(const list_iterator rh) const PLF_NOEXCEPT { return (node_pointer != rh.node_pointer); }

        bool operator!=(const list_iterator<!is_const> rh) const PLF_NOEXCEPT { return (node_pointer != rh.node_pointer); }

        reference operator*() const { return node_pointer->element; }

        pointer operator->() const { return &(node_pointer->element); }
        ...
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type> list_iterator& operator=(const list_iterator<false>& rh) PLF_NOEXCEPT
#else
...
#ifdef PLF_DEFAULT_TEMPLATE_ARGUMENT_SUPPORT
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type> list_iterator(const list_iterator<false>& source) PLF_NOEXCEPT : node_pointer(source.node_pointer) {}
#else
...
#ifdef PLF_DEFAULT_TEMPLATE_ARGUMENT_SUPPORT
        template <bool is_const_it = is_const, class = typename plf::enable_if<is_const_it>::type> list_iterator(const list_iterator<false>&& source) PLF_NOEXCEPT : node_pointer(std::move(source.node_pointer)) {}
#else
    ...
    template <bool is_const_r> class list_reverse_iterator
    {
      ...
      public:
        ...
        bool operator==(const list_reverse_iterator rh) const PLF_NOEXCEPT { return (node_pointer == rh.node_pointer); }

        bool operator==(const list_reverse_iterator<!is_const_r> rh) const PLF_NOEXCEPT { return (node_pointer == rh.node_pointer); }
        ...
        typename list::iterator base() const PLF_NOEXCEPT { return typename list::iterator(node_pointer->next); }
        ...
        list_reverse_iterator(const list_iterator<is_const_r>& source) PLF_NOEXCEPT : node_pointer(source.node_pointer->previous) {}
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        list_reverse_iterator(list_reverse_iterator&& source) PLF_NOEXCEPT : node_pointer(std::move(source.node_pointer)) {}
...
#ifdef PLF_DEFAULT_TEMPLATE_ARGUMENT_SUPPORT
        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type> list_reverse_iterator& operator=(const list_iterator<false>& source) PLF_NOEXCEPT
#else
        list_reverse_iterator& operator=(const list_iterator<!is_const_r>& source) PLF_NOEXCEPT
#endif
        {
...
#ifdef PLF_DEFAULT_TEMPLATE_ARGUMENT_SUPPORT
        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type> list_reverse_iterator& operator=(const list_reverse_iterator<false>& source) PLF_NOEXCEPT
#else
...
#ifdef PLF_DEFAULT_TEMPLATE_ARGUMENT_SUPPORT
        template <bool is_const_rit = is_const_r, class = typename plf::enable_if<is_const_rit>::type> list_reverse_iterator& operator=(list_reverse_iterator<false>&& source) PLF_NOEXCEPT
#else
...
namespace std
{
template <class element_type, class allocator_type> void swap(plf::list<element_type, allocator_type>& a, plf::list<element_type, allocator_type>& b) PLF_NOEXCEPT_SWAP(allocator_type) { a.swap(b); }

template <class element_type, class allocator_type, class predicate_function> typename plf::list<element_type, allocator_type>::size_type erase_if(plf::list<element_type, allocator_type>& container, predicate_function predicate) { return container.remove_if(predicate); }

template <class element_type, class allocator_type> typename plf::list<element_type, allocator_type>::size_type erase(plf::list<element_type, allocator_type>& container, const element_type& value) { return container.remove(value); }

#ifdef PLF_CPP20_SUPPORT
// std::reverse_iterator overload, to allow use of plf::list with ranges and
// make_reverse_iterator primarily:
template <plf::list_iterator_concept it_type> class reverse_iterator<it_type> : public it_type::reverse_type
{
  public:
