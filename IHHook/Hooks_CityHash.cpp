...
namespace IHHook
{
    namespace Hooks_CityHash
    {
        ...
        {
            // string might have a null char anywhere between 0 - str_len, so lets find it and make sure we don't add it to the
            // buffer (because that would stop later buffer entries being written to the log, as the null would come before
            // them)
            ...
            if (str[real_len - 1] == '\0') // above might have missed the null because we had to do str_len - 1, so check again
                                           // just incase (we really don't want to copy any nulls)
                real_len--;
        ...
        unsigned __int64 __fastcall CityHash1Hook(char* str, unsigned int len)
        {
            if (str && len)
            {
                ...
                if (!isPath && a2 > 5)
                {
                    // have to go through the whole string to check, since it seems strings can end with a variable amount of
                    // null bytes
