...
// DEBUGNOW signatures are more robust to game updates/different game versions than straight addresses, but take a long
// time to search since IHHook is started on it's own thread game initialisation will continue, and IHHook wont be ready
// in time to start up IH properly. an alternative would be to do a hook to an early execution point of the game and
// init  ihhook there, but given the low rate of updates of the game it's better to stick with direct addresses, but
// have signatures documented as a backup

// tex macros to declare the various accoutrement required for MH_Hook and straight hooks

// Function addresses are from IDA/Ghidra, which uses the ImageBase field in the exe as the base address (usually
// 0x140000000) the real base address changes every time the game is run though, so we have to remove that base address
// and add the real one so it's rebased when its set up (see GET_REBASED_ADDR for the simple rebasing math)
...
// DEBUGNOW only need one type sig&mask or pattern, but have yet to decide on which to use (sigmaker grabs both so
// there's no real issue on generating though I prefer pattern for easier manual comparing without having to jump back
// and forth between sig and mask, seems all the sigmatching funcs I have currently gathered so far are sig&mask
...
// Expands to:
// MH_STATUS lua_newstateCreateStatus = MH_CreateHook((LPVOID*)addressSet["lua_newstate"], lua_newstateHook,
// (LPVOID*)&lua_newstate); if (lua_newstateCreateStatus != MH_OK) {
