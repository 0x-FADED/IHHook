// lua lib IHH
// TODO: should probably be IhkCore or something to keep in line with rest of
// the lua libs
#include "LuaIHH.h"
...
extern std::map<short, __int64> locationLangIds;

namespace LuaIHH
{
// IHH module funcs>

// tex lua doesnt have any explicit unicode support, so forgoing a more general
// starprocess function via lua for hardcoding to ihext
static int l_StartIHExt(lua_State* L)
{
    ...
    std::wstring commandLine = L" " + gameDir + L" mod mgsvtpp mgsv_in mgsv_out"; // tex seems to need a leading space
                                                                                  // else ihext won't get the first
                                                                                  // arg
...
// By default spdlog is very lazy with it's flush, which really helps
// performance wise But often when you're debugging or developing stuff you want
// to read log updates near real time spdlog does have a periodic flush, but can
// only be used on thread safe loggers (ihh is using st which isn't).
...
// tex DEBUGNOW will have to rethink if we want something else to read the
// messages returns table of string messages from serverPipeIn
...
// tex since the menu is run through the normal game lua update loop can't
// really just have ui call lua directly for actions so it dumps them into a
// queue for the lua menu to grab and process, pretty much the same way pipe in
// messages are handled may have to rethink if expanding ui stuff beyond the IH
// menu returns table of string messages from IHMenu
...
static int l_UpdateChangeLocationMenu(lua_State* L)
{
    ...
    if (!lua_istable(L, -1))
    {
        ...
        { 
            // on stack since nil was pushed
           // uses 'key' (at index -2) and 'value' (at index -1)

            short locationCode = (short)lua_tointeger(L, -2);
            const char* langId = lua_tostring(L, -1);
            luaLog->info("{} = {}", langId, locationCode);

            if (!locationLangIds.contains(locationCode))
            {
                locationLangIds.emplace(locationCode, StrCode64(langId, std::char_traits<char>::length(langId)));
            }
...
// tex TODO better module name, will likely break out into IHH<module name> as
// the amount of functions expands, but would have to change 'if IHH' checks in
// IH
int luaopen_ihh(lua_State* L)
{
    spdlog::debug(__func__);

    luaL_Reg ihh_funcs[] = {{"StartIHExt", l_StartIHExt},
                            {"Log", l_Log},
