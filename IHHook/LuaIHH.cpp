...
namespace IHHook
{
    ...
    namespace LuaIHH
    {
        ...
        // tex lua doesnt have any explicit unicode support, so forgoing a more general starprocess function via lua for
        // hardcoding to ihext
        static int l_StartIHExt(lua_State* L)
        {
            ...
            std::wstring commandLine =
                L" " + gameDir + L" mod mgsvtpp mgsv_in mgsv_out"; // tex seems to need a leading space else ihext won't get the first arg
        ...
        // tex since the menu is run through the normal game lua update loop can't really just have ui call lua directly for
        // actions so it dumps them into a queue for the lua menu to grab and process, pretty much the same way pipe in messages
        // are handled may have to rethink if expanding ui stuff beyond the IH menu returns table of string messages from IHMenu
        ...
        static int l_UpdateChangeLocationMenu(lua_State* L)
        {
            ...
            { // tex locationLangIds param now at second on stack since nil was pushed
                /* uses 'key' (at index -2) and 'value' (at index -1) */
                int locationCode = (int)lua_tointeger(L, -2);
                const char* langId = lua_tostring(L, -1);
                luaLog->info("{}={}", locationCode, langId);
                locationLangIds[locationCode] = ff_stringid_hash_n(langId, strlen(langId));
        ...
        // tex TODO better module name, will likely break out into IHH<module name> as the amount of functions expands, but
        // would have to change 'if IHH' checks in IH
