...
namespace IHHook
{
    ...
    namespace Hooks_Character
    {
        ...
        bool IsPlayerTypeValid(uint playerType)
        {
            ...
            // WORKAROUND: vanilla treats SNAKE/AVATAR the same, and theres some oddness going on when they have different parts
            // in helispace due to the mirror venom/other player instance
        ...
        uint64_t* GetPartsFpkPathHook(uint64_t* fileSlotIndex, uint playerType, uint playerPartsType)
        {
            ...
            if (!IsPlayerTypeValid(playerType) || character.playerPartsFpkPath == "")
            {
                ...
                spdlog::debug(
                    "GetPartsFpkPathHook overrideCharacterSystem:{}, character.playerPartsPartsPath:{}", overrideCharacterSystem, character.playerPartsFpkPath);
            ...
            if (playerPartsType == PlayerPartsType_HOSPITAL || playerPartsType == PlayerPartsType_AVATAR_EDIT_MAN)
            {
                spdlog::debug("GetPartsFpkPathHook is 3 or 14, use vanilla");
                return GetPartsFpkPath(fileSlotIndex, playerType, playerPartsType);
            }

            // DEBUGNOW (WORKAROUND breaks valid use of AVATAR plParts 0) trying to figure out crash problem
            // https://github.com/TinManTex/InfiniteHeaven/issues/32 logging exec flow noticed that loading in to ACC there's an
            // extra call to loadplayer for AVATAR only (before the expected calls to both player instance 0, and 1/AVATAR)
            // which is neither here not there, but for !needHead (talking about underlying property rather than IHH
            // implementation) playerParts it always calls with playerPartsType 0 reguardless of actual playerPartsType. the
            // calls following that have the correct playerPartsType, and playerParts with needHead have the correct
            // playerPartsType
        ...
        uint64_t* GetPartsFilePathHook(uint64_t* fileSlotIndex, uint playerType, uint playerPartsType)
        {
            ...
            if (!IsPlayerTypeValid(playerType) || character.playerPartsPartsPath == "")
            {
                ...
                spdlog::debug(
                    "GetPartsFilePathHook overrideCharacterSystem:{}, character.playerPartsPartsPath:{}",
                    overrideCharacterSystem,
                    character.playerPartsPartsPath);
                overrideCharacterSystem = false;
            }
        ...
        //		//pathsForPlayerType[playerPartsType] = fpkPath;//tex i guess at is returning a new sub map or something because
        // setting it like this doesnt actually change playerPartsFpk 		playerPartsFpk[playerType][playerPartsType] =
        // std::string(filePath);
        //
        //		spdlog::debug("l_SetPlayerPartsFpk set playerType: {} playerPartsType: {} to {}", playerType, playerPartsType,
        // filePath);
        //	}
        ...
        //		spdlog::debug("l_SetPlayerPartsPart set playerType: {} playerPartsType: {} to {}", playerType, playerPartsType,
        // filePath);
        //	}
        //	catch (const std::out_of_range&) {
        ...
        //		}
        //		catch (const std::out_of_range&) {
        //			spdlog::debug("GetPartsFpkPathHook pathsForPlayerType could not find for playerPartsType: {}",
        // playerPartsType); 			filePath64 = 0;
        //		}
        //	}
        //	catch (const std::out_of_range&) {
        ...
        //		}
        //		catch (const std::out_of_range&) {
        //			spdlog::debug("GetPartsFilePathHook pathsForPlayerType could not find for playerPartsType: {}",
        // playerPartsType); 			filePath64 = 0;
        //		}
        //	}
        //	catch (const std::out_of_range&) {
        ...
        bool UseVanillaPlayerCamo(uint playerType, uint playerPartsType, uint playerCamoType)
        {
            ...
            if (playerType == PlayerType_SNAKE || playerType == PlayerType_AVATAR)
            {
                if (PlayerPartsType_SWIMWEAR <= playerPartsType && playerPartsType <= PlayerPartsType_SWIMWEAR_H)
                {
                    return true;
                }
        ...
        // OFF REF
        // ulonglong* GetCamoFilePathHookORIG(ulonglong* fileSlotIndex, uint playerType, uint playerPartsType, uint
        // playerCamoType) { 	spdlog::debug("GetCamoFpkPathHook playerType:{}, playerPartsType:{}", playerType, playerPartsType);
        //	ulonglong fv2Path = 0;
        ...
        ulonglong* GetHandFpkPathHook(ulonglong* fileSlotIndex, uint playerType, uint playerPartsType, uint playerHandType)
        {
            ...
            if (useBionicHand)
            {
                ...
                if (filePath == "")
                {
                    ...
                    // tex WORKAROUND: while turning hand off for partstypes that usually have them works, (ex playerPartsType
                    // NORMAL > playerPartsInfo MGS1) setting a partsType to one that has no hand also sets playerHandType to 0
                    // (ex playerPartsType MGS1 > playerPartsInfo NORMAL) so there must be some other player it defines hand/not
                    // hand per playerType , if not right where the change to playerHandType then likely called there
                    if (playerHandType == PlayerHandType_NONE)
                    {
                        if (overrideCharacterSystem)
                        {
        ...
        // UNUSED REF
        // GOTCHA: since its only called in GetBodyFovaPath, so this isnt a hook, just calling this extended version from
        // GetBodyFovaPathHook only called for playerType 1 DD_MALE, 2 DD_FEMALE bool _DoesNeedBodyFovaForDDORIG(uint
        // playerType, uint playerPartsType) { 	spdlog::debug("_DoesNeedBodyFovaForDDHook playerType:{} playerPartsType:{}",
        // playerType, playerPartsType); 	if (true) { 		switch (playerPartsType) { 		case 0://NORMAL 		case 1://NORMAL_SCARF 		case
        ...
        // DEBUGNOW there's somewhere else filtering whether it's actually applied, ie it still will only apply if correct
        // playerCamoType is set you can test this by setting up char values to a normal camo that supports skintone fv2, and
        // chaning between playerCamoId that supports it or not you'll see this function runs reguardless yet the fv2 is applied
        // or not tomehow posbly theres a flag for that camoID somewhere to use the fv2 variable data 0x64
        // https://metalgearmodding.fandom.com/wiki/FV2#Variable_Data_Section or not but since the foxPathPath file reference
        // doesn't seem to be used past it's call, and there doesn't seem to be any setup function before it (theres other
        // loadfv2 functions), I'm not sure how it would be handled even then it seems to need litterally the exact
        // playerCamoType range (or is it playerpartstype hmm) it was for anyway. again test dd_male swimwear and change it to
        // another skintone supported camo, it dont work.
        ...
        // TODO: figure out how AVATAR is handled, inital look at LoadPlayerFv2s it doesnt seem to use this for AVAT, then what
        // is its skin tone situation? ulonglong* GetBodyFovaPathORIG(ulonglong* fileSlotIndex, uint playerType, uint
        // playerPartsType) { 	spdlog::debug("GetBodyFovaPathHook playerType:{} playerPartsType:{}", playerType,
        // playerPartsType); 	bool shouldApplySkinToneFv2 = false; 	ulonglong filePath64 = 0;
        ...
        // tex ghidra doesn't like to decompile this, but except for ppt 3 / HOSPITAL it seems the same as
        // DoesNeedFaceFovaForAvatarHook GOTCHA: is also called in a bunch of other places, aparently at least one constantly/in
        // the update loop
        bool DoesNeedFaceFovaHook(uint playerPartsType)
        {
            ...
            // OFF, see GOTCHA spdlog::debug("DoesNeedFaceFovaHook playerPartsType:{} headNeeded:{}", playerPartsType,
            // headNeeded); spdlog::debug("DoesNeedFaceFovaHook playerPartsType:{} headNeeded:{} overrideCharacterSystem:{}",
            // playerPartsType, headNeeded,overrideCharacterSystem);
            return headNeeded;
        ...
        bool DoesNeedFaceFovaForAvatarHook(uint playerPartsType)
        {
            ...
            spdlog::debug(
                "DoesNeedFaceFovaForAvatarHook playerPartsType:{} headNeeded:{} overrideCharacterSystem:{}",
                playerPartsType,
                headNeeded,
                overrideCharacterSystem);
        ...
        std::string snakeFaceFv2sDefault[]{
            "/Assets/tpp/fova/chara/sna/sna0_face0_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face1_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face2_v00.fv2",
            "/Assets/tpp/fova/chara/sna/sna0_face4_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face5_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face6_v00.fv2",
        };
            ...
            if (useHead)
            {
                ...
                if (filePath == "")
                {
                    ...
                    else
                    {
                        if (isBandana)
                        {
                            hornLevel = hornLevel + MAX_SNAKE_FACEID; // tex not really right descriptive wise, but we've only
                                                                      // got two 'heads', normal/bandana (* 3 horn levels)
                        }
