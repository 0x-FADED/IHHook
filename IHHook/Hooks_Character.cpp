...
// GOTCHA: AVATAR player parts not being identical to SNAKE cause the change to
// fail to load in ACC something to do with 2nd player instance for the
// 'reflection' i guess does not seem to cause an issue in-mission where there
// is only the singular player instance
...
{
bool overrideCharacterSystem = false; // tex TODO: dont know if I want just an overall 'using ih
                                      // overrides' or per-type override values
...
// playerType 255 = none
// lua l_SetPlayerTypeForPartsType(u32 playerType)
int l_SetPlayerTypeForPartsType(lua_State* L)
{
    character.playerType = (u32)lua_tointeger(L, -1);
...
{
    character.playerPartsType = (u32)lua_tointeger(L, -1);
...
{
    u32 playerHandType = (u32)lua_tointeger(L, -2);
    if (playerHandType == 0)
    {
        // spdlog::warn("l_SetBionicHandFpkPath cannot override playerHandType
        // 0/NONE"); DEBUGNOW return 0;
    }

    if (playerHandType >= MAX_HAND_TYPE)
    { // SILVER
...
int l_SetBionicHandFv2Path(lua_State* L)
{
    ...
    if (playerHandType == 0)
    {
        // spdlog::warn("l_SetBionicHandFv2Path cannot override playerHandType
        // 0/NONE"); DEBUGNOW return 0;
    }
...
// lua SetAvatarHornFpkPath(u32 hornLevel, string fpkPath)
int l_SetAvatarHornFpkPath(lua_State* L)
{
    u32 hornLevel = (u32)lua_tointeger(L, -2);
...
// lua SetAvatarHornFv2Path(u32 hornLevel, string fpkPath)
...
bool IsPlayerTypeValid(u32 playerType)
{
    ...
    // WORKAROUND: vanilla treats SNAKE/AVATAR the same, and theres some oddness
    // going on when they have different parts in helispace due to the mirror
    // venom/other player instance
...
bool IsPlayerPartsTypeValid(u32 playerPartsType)
{
...
uint64_t* LoadPlayerPartsFpkHook(uint64_t* fileSlotIndex, u32 playerType, u32 playerPartsType)
{
    ...
    {
        // DEBUGNOW ASSUMPTION: this being the first extended function were
        // hooking tex turn it off entirely if it doesnt match DEBUGNOW the
        // funcs not gated by overrideCharacterSystem
    ...
    // DEBUGNOW (WORKAROUND breaks valid use of AVATAR plParts 0) trying to
    // figure out crash problem
    // https://github.com/TinManTex/InfiniteHeaven/issues/32 logging exec flow
    // noticed that loading in to ACC there's an extra call to loadplayer for
    // AVATAR only (before the expected calls to both player instance 0, and
    // 1/AVATAR) which is neither here not there, but for !needHead (talking
    // about underlying property rather than IHH implementation) playerParts it
    // always calls with playerPartsType 0 reguardless of actual
    // playerPartsType. the calls following that have the correct
    // playerPartsType, and playerParts with needHead have the correct
    // playerPartsType
...
uint64_t* LoadPlayerPartsPartsHook(uint64_t* fileSlotIndex, u32 playerType, u32 playerPartsType)
{
    ...
    {
        // tex as above, but to catch odd cases (LoadPlayerPartsParts is called
        // on mission load without LoadPlayerPartsFpk)
        overrideCharacterSystem = false;
    }
...
std::map<u32, std::map<u32, std::string>> playerPartsFpk = {
    {
        0,
        {
            // SNAKE
            // manual tests
            //{4,"/Assets/tpp/pack/player/parts/plparts_ninja.fpk"},//4/MGS1 >
            // ninja test swap
            //{24,"/Assets/tpp/pack/player/parts/plparts_ocelot.fpk"}//24/non
            // existant partsTypeEnum test
        }, // MGS snake
    },
    {
        1,
...
std::map<u32, std::map<u32, std::string>> playerPartsParts = {
    {
        0,
        {
            // SNAKE
            //{4,"/Assets/tpp/parts/chara/nin/nin0_main0_def_v00.parts"},//MGS1
            //> ninja test swap
            //{284,"/Assets/tpp/parts/chara/ooc/ooc0_main1_def_v00.parts"}//28/non
            // existant partsTypeEnum test
        }, // MGS snake
    },
    {
...
// input: u32 playerType, u32 playerPartsType, string fpkPath
...
//	u32 playerType = (u32)lua_tointeger(L, -3);
//	u32 playerPartsType = (u32)lua_tointeger(L, -2);
//	const char* filePath = lua_tostring(L, -1);

//	if (playerType > 255) {
//		spdlog::error("l_SetPlayerPartsFpk set playerType > max value of
// 255"); 		return 0;
//	}
//	if (playerPartsType > 255) {
//		spdlog::error("l_SetPlayerPartsFpk set playerPartsType > max
// value of 255"); 		return 0;
//	}

//	try {
//		auto pathsForPlayerType = playerPartsFpk.at(playerType);
//		//TODO: log existing path if its being overwritten
//		//pathsForPlayerType[playerPartsType] = fpkPath;//tex i guess at
// is returning a new sub map or something because setting it like this doesnt
// actually change playerPartsFpk
// playerPartsFpk[playerType][playerPartsType] = std::string(filePath);
//
//		spdlog::debug("l_SetPlayerPartsFpk set playerType: {}
// playerPartsType: {} to {}", playerType, playerPartsType, filePath);
//	}
//	catch (const std::out_of_range&) {
//		spdlog::debug("l_SetPlayerPartsFpk playerPartsFpk could not find
...
//	if (playerType > 255) {
//		spdlog::error("l_SetPlayerPartsPart set playerType > max value
// of 255"); 		return 0;
//	}
//	if (playerPartsType > 255) {
//		spdlog::error("l_SetPlayerPartsPart set playerPartsType > max
// value of 255"); 		return 0;
//	}
...
//		//TODO: log existing path if its being overwritten
//		playerPartsParts[playerType][playerPartsType] =
// std::string(filePath); 		spdlog::debug("l_SetPlayerPartsPart set
// playerType: {} playerPartsType: {} to {}", playerType, playerPartsType,
// filePath);
//	}
//	catch (const std::out_of_range&) {
//		spdlog::debug("l_SetPlayerPartsPart playerPartsParts could not
// find for playerType: {}", playerType);
//	}

//	return 0;
//}//l_SetPlayerPartsPart

// tex OFF a better way to allow swapping and extending to other playerPartsType
// values but because of that will hit the saved at no longer valid value if
// user uninstalls mod problem. uint64_t* LoadPlayerPartsFpkAlt(uint64_t*
// fileSlotIndex, u32 playerType, u32 playerPartsType) {
//	spdlog::debug("LoadPlayerPartsFpk playerType:{}, playerPartsType:{}",
// playerType, playerPartsType); 	uint64_t filePath64 = 0;
...
//		}
//		catch (const std::out_of_range&) {
//			spdlog::debug("LoadPlayerPartsFpkHook pathsForPlayerType
// could not find for playerPartsType: {}", playerPartsType);
// filePath64 = 0;
//		}
//	}
//	catch (const std::out_of_range&) {
//		spdlog::debug("LoadPlayerPartsFpkHook playerPartsFpk could not
// find for playerType: {}", playerType);
//	}
//	//tex fall back to original function
...
// uint64_t* LoadPlayerPartsPartsAlt(uint64_t* fileSlotIndex, u32 playerType,
// u32 playerPartsType) { 	spdlog::debug("LoadPlayerPartsPartsHook
// playerType:{}, playerPartsType:{}", playerType, playerPartsType); 	uint64_t
// filePath64 = 0;
...
//		}
//		catch (const std::out_of_range&) {
//			spdlog::debug("LoadPlayerPartsPartsHook
// pathsForPlayerType could not find for playerPartsType: {}", playerPartsType);
//			filePath64 = 0;
//		}
//	}
//	catch (const std::out_of_range&) {
//		spdlog::debug("LoadPlayerPartsPartsHook playerPartsParts could
// not find for playerType: {}", playerType);
//	}
//	//tex fall back to original function
...
// OFF REF
// u64* LoadPlayerCamoFpkORIG(u64* fileSlotIndex, u32 playerType,
// u32 playerPartsType, u32 playerCamoType) {
//	spdlog::debug("LoadPlayerCamoFpkHook playerType:{}, playerPartsType:{}",
// playerType, playerPartsType); 	uint64_t fpkPath = 0;

//	if ((playerType == 0) || (playerType == 3)) {//SNAKE, AVATAR
//		//ORIG
//		if ((20 < playerPartsType - 2) && (playerPartsType < 26)) {
//		//playerPartsType - 2 means 0 NORMAL and 1 SCARF will underflow
// u32 playerPartsType to FFFFF/E,
//		//so 20 < is true
//		//and then on the other end of the range 23 SWIMWEAR (and above)
//- 2 == 21 which is 20 <
//		//playerPartsType < 26 OCELLOT is current playerPartsType max
//		//but after all that, snake/avat don't have swimsuit and just
// has default fatigues for those entries
//		//DEBRAINTEASED
//		//if ((playerPartsType < 2) || (playerPartsType > 22 &&
// playerPartsType < 26)) { 			fpkPath =
//(&SnakeNormalCamoFpkArray_DAT_142a80a10)[(uint64_t)playerCamoType * 2];
// return LoadPlayerCamoFpk(fileSlotIndex, playerType, playerPartsType,
// playerCamoType);
//		}

//		if (playerPartsType == 7) {//NAKED
//			fpkPath =
//(&SnakeNakedCamoFpkArray_DAT_142a81160)[(uint64_t)playerCamoType * 2];
// return LoadPlayerCamoFpk(fileSlotIndex, playerType, playerPartsType,
// playerCamoType);
//		}
//	}
...
//			fpkPath =
//(&DDMaleCamoFpkArray_DAT_142a818b0)[(uint64_t)playerCamoType * 2];
// return LoadPlayerCamoFpk(fileSlotIndex, playerType, playerPartsType,
// playerCamoType);
//		}
//		if (playerType == 2) {//DD_FEMALE
//			fpkPath =
//(&DDFemaleCamoFpkArray_DAT_142a82000)[(uint64_t)playerCamoType * 2];
// return LoadPlayerCamoFpk(fileSlotIndex, playerType, playerPartsType,
// playerCamoType);
//		}
//	}
...
bool UseVanillaPlayerCamo(u32 playerType, u32 playerPartsType, u32 playerCamoType)
{
...
u64* LoadPlayerCamoFpkHook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 playerCamoType)
{
    ...
    { // tex I guess 255 is NONE/not set.
...
u64* LoadPlayerCamoFv2Hook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 playerCamoType)
{
...
// OFF REF
// u64* LoadPlayerCamoFv2HookORIG(u64* fileSlotIndex, u32
// playerType, u32 playerPartsType, u32 playerCamoType) {
//	spdlog::debug("LoadPlayerCamoFpkHook playerType:{}, playerPartsType:{}",
// playerType, playerPartsType); 	u64 fv2Path = 0;
...
// playerPartsType < 26)) {
//			//DEBUGNOW fv2Path =
//(&SnakeNormalCamoFv2Array_DAT_142a80a18)[(u64)playerCamoType * 2];
//		}
//		if (playerPartsType == 7) {//NAKED
//			//DEBUGNOW fv2Path =
//(&SnakeNakedCamoFv2Array_DAT_142a81168)[(u64)playerCamoType * 2];
//		}
//	}
//	else {
//		if (playerType == 1) {//DD_MALE
//			//DEBUGNOW fv2Path =
//(&DDMaleCamoFv2Array_DAT_142a818b8)[(u64)playerCamoType * 2];
//		}
//		if (playerType == 2) {//DD_FEMALE
//			//DEBUGNOW fv2Path =
//(&DDFemaleCamoFv2ArrayDAT_142a82008)[(u64)playerCamoType * 2];
//		}
//	}
...
// TODO: there's also facialhelispace to deal with before I'm happy with
// extending this
u64* LoadPlayerFacialMotionFpkHook(u64* fileSlotIndex, u32 playerType)
{
    spdlog::debug("LoadPlayerFacialMotionFpkHook playerType:{}", playerType);
    if (playerType == 1)
    { // DD_MALE
        LoadFile(fileSlotIndex,
                 0x522bba0fe696843e); // /Assets/tpp/pack/player/motion/player2_facial_dd_male.fpk
    }
    else
    {
        if (playerType == 2)
        { // DD_FEMALE
            LoadFile(fileSlotIndex,
                     0x5228819af53ce132); // /Assets/tpp/pack/player/motion/player2_facial_dd_female.fpk
        }
        else
        {
            if (playerType == 5)
            { // OCELOT
                LoadFile(fileSlotIndex,
                         0x522ad6eb108b656a); // /Assets/tpp/pack/player/motion/player2_facial_ocelot.fpk
            }
            else
            {
                if (playerType == 6)
                { // QUIET
                    LoadFile(fileSlotIndex,
                             0x522ad26ea9839391); // /Assets/tpp/pack/player/motion/player2_facial_quiet.fpk
                }
                else
                { // SNAKE,AVATAR (default vanilla), LIQUID
                    LoadFile(fileSlotIndex,
                             0x522a1da4adfd5137); // /Assets/tpp/pack/player/motion/player2_facial_snake.fpk
                }
            }
        }
    }
...
u64* LoadPlayerFacialMotionMtarHook(u64* fileSlotIndex, int playerType)
{
    spdlog::debug("LoadPlayerFacialMotionMtarHook playerType:{}", playerType);
    if (playerType == 1)
    {
        LoadFile(fileSlotIndex,
                 0x67026b0d3dfd05e2); // /Assets/tpp/motion/mtar/player2/player2_ddm_facial.mtar
    }
    else
    {
        if (playerType == 2)
        {
            LoadFile(fileSlotIndex,
                     0x670245b34a1d710c); // /Assets/tpp/motion/mtar/player2/player2_ddf_facial.mtar
        }
        else
        {
            if (playerType == 5)
            {
                LoadFile(fileSlotIndex,
                         0x6703e118275df4f2); // /Assets/tpp/motion/mtar/player2/player2_ocelot_facial.mtar
            }
            else
            {
                if (playerType == 6)
                {
                    LoadFile(fileSlotIndex,
                             0x6701511616076078); // /Assets/tpp/motion/mtar/player2/player2_quiet_facial.mtar
                }
                else
                {
                    LoadFile(fileSlotIndex,
                             0x67028b3526a03df4); // /Assets/tpp/motion/mtar/player2/TppPlayer2Facial.mtar
                }
            }
        }
    }
...
std::string bionicHandFv2Paths[]{
    "", "/Assets/tpp/fova/chara/sna/sna0_arm0_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm3_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm4_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm2_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm1_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm6_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_arm7_v00.fv2",
};

// tex default values from LoadPlayerBionicArm
// SYNC vanilla
bool UseBionicArmVanilla(u32 playerType, u32 playerPartsType, u32 playerHandType)
{
    // SNAKE,AVATAR
    if (playerType == 0 || playerType == 3)
    {
...
u64* LoadPlayerBionicArmFpkHook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 playerHandType)
{
    ...
    u64 filePath64 = 0; // tex 0 acts as unload/no hand, vanilla has this
                              // for 0/NONE index in its fpk/fv2 path64 array
    if (useBionicHand)
    {
        // tex .. but the actual hand type is independant of
        // overrideCharacterSystem
        std::string filePath = character.bionicHandFpkPath;
        if (filePath == "")
        {
            // tex vanilla paths
            // TODO: surface this information to player if nessesary
            // tex WORKAROUND: while turning hand off for partstypes that
            // usually have them works, (ex playerPartsType NORMAL >
            // playerPartsInfo MGS1) setting a partsType to one that has no hand
            // also sets playerHandType to 0 (ex playerPartsType MGS1 >
            // playerPartsInfo NORMAL) so there must be some other player it
            // defines hand/not hand per playerType , if not right where the
            // change to playerHandType then likely called there
            if (playerHandType == 0)
            {
                if (overrideCharacterSystem)
                {
...
u64* LoadPlayerBionicArmFv2Hook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 playerHandType)
{
...
bool CheckPlayerPartsIfShouldApplySkinToneFv2Hook(u32 playerType, u32 playerPartsType)
{
...
// UNUSED REF
// GOTCHA: since its only called in LoadPlayerPartsSkinToneFv2, so this isnt a
// hook, just calling this extended version from LoadPlayerPartsSkinToneFv2Hook
// only called for playerType 1 DD_MALE, 2 DD_FEMALE
// bool CheckPlayerPartsIfShouldApplySkinToneFv2ORIG(u32 playerType, u32
// playerPartsType) {
//	spdlog::debug("CheckPlayerPartsIfShouldApplySkinToneFv2Hook
// playerType:{} playerPartsType:{}", playerType, playerPartsType); 	if
// (true) { 		switch (playerPartsType) { 		case 0://NORMAL
// case 1://NORMAL_SCARF
//		case 2://SNEAKING_SUIT
//		case 7://NAKED
//		case 8://SNEAKING_SUIT_TPP
...
// DEBUGNOW there's somewhere else filtering whether it's actually applied, ie
// it still will only apply if correct playerCamoType is set you can test this
// by setting up char values to a normal camo that supports skintone fv2, and
// chaning between playerCamoId that supports it or not you'll see this function
// runs reguardless yet the fv2 is applied or not tomehow posbly theres a flag
// for that camoID somewhere to use the fv2 variable data 0x64
// https://metalgearmodding.fandom.com/wiki/FV2#Variable_Data_Section or not but
// since the LoadFile file reference doesn't seem to be used past it's call, and
// there doesn't seem to be any setup function before it (theres other loadfv2
// functions), I'm not sure how it would be handled even then it seems to need
// litterally the exact playerCamoType range (or is it playerpartstype hmm) it
// was for anyway. again test dd_male swimwear and change it to another skintone
// supported camo, it dont work.
u64* LoadPlayerPartsSkinToneFv2Hook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType)
{
    spdlog::trace(__func__);
    if (!overrideCharacterSystem)
    {
...
// TODO: expand. fill out all the data taking
// CheckPlayerPartsIfShouldApplySkinToneFv2 into account then assume if value
// then apply and CheckPlayerPartsIfShouldApplySkinToneFv2 will no longer be
// nessesary
// TODO: figure out how AVATAR is handled, inital look at LoadPlayerFv2s it
// doesnt seem to use this for AVAT, then what is its skin tone situation?
// u64* LoadPlayerPartsSkinToneFv2ORIG(u64* fileSlotIndex, u32
// playerType, u32 playerPartsType) {
//	spdlog::debug("LoadPlayerPartsSkinToneFv2Hook playerType:{}
// playerPartsType:{}", playerType, playerPartsType); 	bool
// shouldApplySkinToneFv2 = false; 	u64 filePath64 = 0;

//	if (playerType == 0) {//SNAKE
//		if (playerPartsType == 18) {//MGS3_TUXEDO
//			filePath64 =
// 0x608961e868491c54;////"/Assets/tpp/fova/chara/dld/dld0_main0_sna.fv2";
//		}
//	} else if (playerType == 1) {//DD_MALE
//		shouldApplySkinToneFv2 =
// CheckPlayerPartsIfShouldApplySkinToneFv2(playerType, playerPartsType);
// if (shouldApplySkinToneFv2) { 			switch (playerPartsType)
//{ 			case 8://SNEAKING_SUIT_TPP
// filePath64 = 0x608b9ec8eac8437b;//
//"/Assets/tpp/fova/chara/sna/sna4_plym0_def_v00.fv2";
// break; 			case 9://BATTLEDRESS
// filePath64 = 0x608b9ec8eac8437b;//
//"/Assets/tpp/fova/chara/sna/sna4_plym0_def_v00.fv2";
// break; 			case 15://MGS3 filePath64 =
// 0x608b3a2e8398415b;//
//"/Assets/tpp/fova/chara/dla/dla0_plym0_v00.fv2";
// break; 			case 16://MGS3_NAKED
// filePath64 = 0x608bed35c90a314d;//
//"/Assets/tpp/fova/chara/dla/dla1_plym0_v00.fv2";
// break; 			case 18://MGS3_TUXEDO
// filePath64 = 0x608872bab5e53bc8; //
//"/Assets/tpp/fova/chara/dld/dld0_plym0_v00.fv2";
// break; 			case 23://SWIMWEAR
// filePath64 = 0x608aa0de59bf9572; //
//"/Assets/tpp/fova/chara/dlf/dlf1_main0_v00.fv2";
// break; 			case 24://SWIMWEAR_G
// filePath64 = 0x6088dd7cacaa3fd6; //
//"/Assets/tpp/fova/chara/dlg/dlg1_main0_v00.fv2";
// break; 			case 25://SWIMWEAR_H
// filePath64 = 0x60884821796ed8f0;//
//"/Assets/tpp/fova/chara/dlh/dlh1_main0_v00.fv2";
// break; 			default: filePath64 =
// 0x608882ccbb15c7ab;//"/Assets/tpp/fova/chara/sna/dds5_main0_ply_v00.fv2"
//				break;
//			}//switch(playerPartsType)
//		}//shouldApplySkinToneFv2
...
// filePath64 = 0x608b9ec8eac8437b;//
//"/Assets/tpp/fova/chara/sna/sna4_plym0_def_v00.fv2";
// break; 			case 19://EVA_CLOSE
// filePath64 = 0x608bc54842becde0;//
//"/Assets/tpp/fova/chara/dle/dle0_plyf0_v00.fv2";
// break; 			case 20://EVA_OPEN
// filePath64 = 0x608a91e3d60c5980;//
//"/Assets/tpp/fova/chara/dle/dle1_plyf0_v00.fv2";
// break; 			case 22://BOSS_OPEN
// filePath64 = 0x6089e156b2cacad9;//
//"/Assets/tpp/fova/chara/dlc/dlc1_plyf0_v00.fv2";
// break; 			case 23://SWIMWEAR
// filePath64 = 0x6088fc6455404f89;//
//"/Assets/tpp/fova/chara/dlf/dlf1_main0_f_v00.fv2";
// break; 			case 24://SWIMWEAR_G
// filePath64 = 0x6089659d7ee7f080;//
//"/Assets/tpp/fova/chara/dlg/dlg1_main0_f_v00.fv2";
// break; 			case 25://SWIMWEAR_H
// filePath64 = 0x6089e8ede46843e9; //
//"/Assets/tpp/fova/chara/dlh/dlh1_main0_f_v00.fv2";
// break; 			default: filePath64 = 0x608a1c34fefc05c2;//
//"/Assets/tpp/fova/chara/sna/dds6_main0_ply_v00.fv2";
// break;
//			}//switch(playerPartsType)
//		}//shouldApplySkinToneFv2
//	}//if playerType
...
// tex ghidra doesn't like to decompile this, but except for ppt 3 / HOSPITAL it
// seems the same as IsHeadNeededForPartsTypeAndAvatarHook GOTCHA: is also
// called in a bunch of other places, aparently at least one constantly/in the
// update loop
bool IsHeadNeededForPartsTypeHook(u32 playerPartsType)
{
    // DEBUG
    /*for (u32 i = 0; i < 28; i++) {
    ...
    // OFF, see GOTCHA spdlog::debug("IsHeadNeededForPartsTypeHook
    // playerPartsType:{} headNeeded:{}", playerPartsType, headNeeded);
    return headNeeded;
} // IsHeadNeededForPartsTypeHook

// AVATAR only? VERIFY
bool IsHeadNeededForPartsTypeAndAvatarHook(u32 playerPartsType)
{
    // DEBUGNOW
    /*for (u32 i = 0; i < 28; i++) {
            bool testHead = IsHeadNeededForPartsType(i);
            spdlog::debug("IsHeadNeededForPartsTypeAndAvatarHook {} = {}", i,
    testHead);
    }*/
...
std::string snakeFaceFv2sDefault[]{
    "/Assets/tpp/fova/chara/sna/sna0_face0_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face1_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face2_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face4_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face5_v00.fv2", "/Assets/tpp/fova/chara/sna/sna0_face6_v00.fv2",
};

// tex broken out from LoadPlayerSnakeFace
// essentially IsHeadNeededForPartsTypeAndSnake
// REF UNUSED
bool UsePlayerSnakeFaceVanilla(u32 playerType, u32 playerPartsType)
{
    switch (playerPartsType)
    {
    case 0:  // NORMAL
...
// tex vanilla does not have seperate IsHeadNeededForPartsTypeSnake, is rolled
// into LoadPlayerSnakeFaceFpk for playerType SNAKE it uses playerFaceId for
// hornLevel
u64* LoadPlayerSnakeFaceFpkHook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 hornLevel, char playerFaceEquipId)
{
    ...
    if (useHead)
    {
        // tex .. but what face is used is independant from
        // overrideCharacterSystem
        ...
        if (filePath == "")
        {
            ...
            if (isBandana)
            {
                hornLevel = hornLevel + MAX_SNAKE_FACEID; // tex not really right descriptive
                                                          // wise, but we've only got two 'heads',
                                                          // normal/bandana (* 3 horn levels)
            }
...
u64* LoadPlayerSnakeFaceFv2Hook(u64* fileSlotIndex, u32 playerType, u32 playerPartsType, u32 hornLevel, char playerFaceEquipId)
{
    spdlog::debug("LoadPlayerSnakeFaceFpkHook playerPartsType:{} headNeeded:{}", playerPartsType, character.useHead);
...
u64* LoadAvatarOgreHornFpkHook(u64* fileSlotIndex, u32 ogreLevel)
{
    u64 filePath64 = 0;
...
u64* LoadAvatarOgreHornFv2Hook(u64* fileSlotIndex, u32 ogreLevel)
{
    u64 filePath64 = 0;
