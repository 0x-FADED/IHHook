...
pref_done:
    ...
    if (c == 0xc4 || c == 0xc5 || c == 0x62 || (c == 0x8f && p < limit && (*p & 0x1f) >= 8))
    {
        uint8_t map;
        ...
        if (c == 0x8f)
        {
            ...
            {
            case 1: /* 0F map */
                if (opcode == 0x70 || (opcode >= 0x71 && opcode <= 0x73) || opcode == 0xc2 || (opcode >= 0xc4 && opcode <= 0xc6))
                    cflags |= C_IMM8;
            ...
            case 3: /* 0F 3A map: always an imm8 */
                ...
                break;
            case 5:
            case 6: /* half-precision maps, EVEX only */
                if (c != 0x62)
                    hs->flags |= F_ERROR | F_ERROR_OPCODE;
                else if (
                    map == 5
                    && (opcode == 0x08 || opcode == 0x0a || opcode == 0x26 || opcode == 0x27 || opcode == 0x56 || opcode == 0x57 || opcode == 0x66
                        || opcode == 0x67 || opcode == 0xc2))
    ...
    if (c == 0xd5)
    {
        ...
        if (c == 0x0f || (c & 0xf0) == 0x40 || c == 0x26 || c == 0x2e || c == 0x36 || c == 0x3e || c == 0x64 || c == 0x65 || c == 0x66 || c == 0x67 || c == 0xf0
            || c == 0xf2 || c == 0xf3)
            hs->flags |= F_ERROR | F_ERROR_OPCODE;
        if (hs->rex_w && (c & 0xf8) == 0xb8)
...
opcode_ready:
    ...
    if (cflags == C_ERROR)
    {
        ...
        cflags = 0;
        if (map0f && (opcode == 0x0b || (opcode == 0x37 && mand == PRE_NONE)))
        {
            ; /* ud2 and getsec take no operand */
        }
        else if (map0f && (opcode == 0xa6 || opcode == 0xa7 || opcode == 0xb9 || opcode == 0xff))
        {
    ...
    if (map0f)
    {
         ...
         * table. */
        if ((ht[ht[opcode / 4] + (opcode % 4)] & mand) && !(opcode == 0x2b && (mand & (PRE_F2 | PRE_F3))))
            hs->flags |= F_ERROR | F_ERROR_OPCODE;
