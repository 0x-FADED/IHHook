...
plf::list<foxStringReplace> replaceStrings = {};
...
// GOTCHA: as this is called most frames at mission runtime (turn on logging to
// see, seems to be repeated foxstring creations that you would have expected
// kjp to have optimized out) best use case of replacement is during mission
// load/before mission runtime to avoid potential performance hit of string
// matches.
fox::String* CreateInPlaceHook(fox::String* outFoxString, char* cString)
{
    ...
    {
        plf::list<foxStringReplace>::iterator it;
        for (it = replaceStrings.begin(); it != replaceStrings.end(); ++it)
        {
            ...
            if (it->subStringReplacement)
            {
                ...
                {
                    scString.erase(subString,
                                   scString.length()); // ZIP: Remove what matches.
                    isMatched = true;
                }
            }
            ...
            if (isMatched)
            {
                ...
                if (it->removeAfterReplacement == true)
                {
                    ...
                    if (std::size(replaceStrings) <= 0)
                    {
                        ...
                        hasStringReplacements = false; // ZIP: Disables when there are no
                                                       // replacements.
                    }
                }
...
bool ParseConfig(std::string fileName)
{
    ...
    while (std::getline(infile, line))
    {
        ...
        // ZIP: If there are any valid entries, enable override and add entry to
        // list.
        if (varName != "" && valueStr != "")
        {
