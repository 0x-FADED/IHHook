...
void CreateTestTable(lua_State* L)
{
    ...
    {
        // tex not really doing much, setting a key to nil is the same as a key
        // that wasnt set lol so the assert should be ok for a simeple test
...
void TestHooks_Lua(lua_State* L)
{
    ...
    */
    ...
    // tex cant think of a good test for this right now (but really these tests
    // are more about seeing if the hooked function actually calls/doesn't crash
    // rather than testing the actual lua api correctness)
    ...
    lua_pop(L,
            1); //_IHHook_TestTable, doubles as lua_settop test (well it would
                // if we hadn't already used pop before this point)
    ...
    while (lua_next(L, -2) != 0)
    {
        ...
        spdlog::debug("{} - {}", lua_typename(L, lua_type(L, -2)), lua_typename(L, lua_type(L, -1)));
