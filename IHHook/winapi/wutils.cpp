...
#include <string.h>
#include <windows.h>
// tex a few edits, search //tex
#define MAX_KEY MAX_PATH

#include "wutils.hpp" //tex was .h
...
void release_ref(lua_State* L, Ref ref) { luaL_unref(L, LUA_REGISTRYINDEX, ref); }
...
// @function push_error
int push_error(lua_State* L) { return push_error_msg(L, last_error(0)); }
...
// @return 2; 'nil' and the message
// @function push_error
int push_error_code(lua_State* L, int err) { return push_error_msg(L, last_error(err)); }
...
void make_message_window()
{
    if (hMessageWin == NULL)
    {
        ...
        s_use_mutex = FALSE;
        hMessageWin = CreateWindowA( // tex was CreateWindow
            "STATIC", "winapi_Spawner_Dispatcher", 0, 0, 0, 0, 0, 0, 0, GetModuleHandle(NULL), 0);
...
void lock_mutex() { WaitForSingleObject(hMutex, INFINITE); }

void setup_mutex() { hMutex = CreateMutex(NULL, TRUE, NULL); }

void release_mutex() { ReleaseMutex(hMutex); }
...
/// call a Lua function.
// This ensures that only one Lua function can be entered at any time,
// controlled by a mutex. If in 'GUI mode' then the Lua function is furthermore
// called from the GUI state.
// @param L the state
...
void set_encoding(int e) { current_encoding = e; }

/// get the encoding.
// @return current encoding
// @function set_encoding
int get_encoding() { return current_encoding; }
...
{
    int res = MultiByteToWideChar(current_encoding, 0, text, -1, wbuf, bufsz);
    if (res != 0)
    {
        return wbuf;
    }
...
{
    int osz = 3 * len;
    char* obuff = (char*) malloc(osz);
    int res = WideCharToMultiByte(current_encoding, 0, us, len, obuff, osz, NULL, NULL);
    if (res == 0)
    {
        free(obuff);
        return push_error(L);
...
int mb_const(LPCSTR name)
{
    if (eq(name, "ok"))
        return MB_OK;
    if (eq(name, "abort-retry-ignore"))
        return MB_ABORTRETRYIGNORE;
    if (eq(name, "ok-cancel"))
        return MB_OKCANCEL;
    if (eq(name, "retry-cancel"))
        return MB_RETRYCANCEL;
    if (eq(name, "yes-no"))
        return MB_YESNO;
    if (eq(name, "yes-no-cancel"))
        return MB_YESNOCANCEL;
    // icons
    if (eq(name, "warning"))
        return MB_ICONWARNING;
    if (eq(name, "information"))
