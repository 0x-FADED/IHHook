// Repo: https://github.com/stevedonovan/winapi
// Docs: https://stevedonovan.github.io/winapi/api.html
// tex a few edits, commented out #line winapi.l.cs, *this to *thisW
/***
...
#undef UNICODE                  // tex WORKAROUND, IHHook is set to unicode
#pragma comment(lib, "mpr.lib") // WNetGetConnect tex
// OFF #line 16 "winapi.l.c"
...
// OFF #line 43 "winapi.l.c"

#include "wutils.hpp" //tex

static WStr wstring(Str text) { return wstring_buff(text, wbuff, sizeof(wbuff)); }
...
// @param text ASCII text with #XXXX, where XXXX is four hex digits. ## means #
// itself.
...
const DWORD_PTR WIN_NOACTIVATE = (DWORD_PTR) SWP_NOACTIVATE, WIN_NOMOVE = (DWORD_PTR) SWP_NOMOVE, WIN_NOSIZE = (DWORD_PTR) SWP_NOSIZE, WIN_SHOWWINDOW = (DWORD_PTR) SWP_SHOWWINDOW, WIN_NOZORDER = (DWORD_PTR) SWP_NOZORDER, WIN_BOTTOM = (DWORD_PTR) HWND_BOTTOM, WIN_NOTOPMOST = (DWORD_PTR) HWND_NOTOPMOST, WIN_TOP = (DWORD_PTR) HWND_TOP, WIN_TOPMOST = (DWORD_PTR) HWND_TOPMOST;

/// a class representing a Window.
// @type Window
// OFF #line 152 "winapi.l.c"
...
{
    Window* thisW = (Window*) luaL_checkudata(L, idx, Window_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Window expected");
    return thisW;
}

static void Window_ctor(lua_State* L, Window* thisW, HWND h);

static int push_new_Window(lua_State* L, HWND h)
{
    Window* thisW = (Window*) lua_newuserdata(L, sizeof(Window));
    luaL_getmetatable(L, Window_MT);
    lua_setmetatable(L, -2);
    Window_ctor(L, thisW, h);
    return 1;
}

static void Window_ctor(lua_State* L, Window* thisW, HWND h)
{
    // OFF #line 153 "winapi.l.c"
    thisW->hwnd = h;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 168 "winapi.l.c"
    lua_pushnumber(L, (DWORD_PTR) thisW->hwnd);
    return 1;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 175 "winapi.l.c"
    GetWindowTextW(thisW->hwnd, wbuff, sizeof(wbuff));
...
static int l_Window_set_text(lua_State* L)
{
    ...
    // OFF #line 182 "winapi.l.c"
    SetWindowTextW(thisW->hwnd, wstring(text));
    return 0;
}
...
static int l_Window_show(lua_State* L)
{
    ...
    // OFF #line 190 "winapi.l.c"
    ShowWindow(thisW->hwnd, flags);
    return 0;
}
...
static int l_Window_show_async(lua_State* L)
{
    Window* thisW = Window_arg(L, 1);
    int flags = luaL_optinteger(L, 2, SW_SHOW);
    // OFF #line 198 "winapi.l.c"
    ShowWindowAsync(thisW->hwnd, flags);
    return 0;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 207 "winapi.l.c"
    RECT rect;
    GetWindowRect(thisW->hwnd, &rect);
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 229 "winapi.l.c"
    lua_pushboolean(L, IsWindowVisible(thisW->hwnd));
    return 1;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 236 "winapi.l.c"
    DestroyWindow(thisW->hwnd);
    return 0;
}
...
static int l_Window_resize(lua_State* L)
{
    ...
    // OFF #line 247 "winapi.l.c"
    MoveWindow(thisW->hwnd, x0, y0, w, h, TRUE);
    return 0;
}

/// resize or move a window.
// see
// [API](http://msdn.microsoft.com/en-us/library/windows/desktop/ms633545%28v=vs.85%29.aspx)
...
// @param h height
// @param flags one of
// WIN_NOACTIVATE, WIN_NOMOVE, WIN_NOSIZE, WIN_SHOWWINDOW (default),
// WIN_NOZORDER
static int l_Window_set_pos(lua_State* L)
{
    ...
    // OFF #line 262 "winapi.l.c"
    SetWindowPos(thisW->hwnd, (HWND) (DWORD_PTR) wafter, x0, y0, w, h, flags);
    return 0;
}
...
static int l_Window_send_message(lua_State* L)
{
    ...
    // OFF #line 273 "winapi.l.c"
    lua_pushinteger(L, SendMessage(thisW->hwnd, msg, (WPARAM) wparam, (LPARAM) lparam));
    return 1;
}
...
static int l_Window_post_message(lua_State* L)
{
    ...
    double wparam = luaL_checknumber(L, 3);
    double lparam = luaL_checknumber(L, 4);
    // OFF #line 284 "winapi.l.c"
    return push_bool(L, PostMessage(thisW->hwnd, msg, (WPARAM) wparam, (LPARAM) lparam));
}
...
static int l_Window_enum_children(lua_State* L)
{
    Window* thisW = Window_arg(L, 1);
    int callback = 2;
    // OFF #line 292 "winapi.l.c"
    Ref ref;
    sL = L;
    ref = make_ref(L, callback);
    EnumChildWindows(thisW->hwnd, &enum_callback, ref);
    release_ref(L, ref);
    return 0;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 303 "winapi.l.c"
    return push_new_Window(L, GetParent(thisW->hwnd));
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 309 "winapi.l.c"
    int sz = GetWindowModuleFileNameW(thisW->hwnd, wbuff, sizeof(wbuff));
    wbuff[sz] = 0;
    return push_wstring(L, wbuff);
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 319 "winapi.l.c"
    static char buff[1024];
    int n = GetClassName(thisW->hwnd, buff, sizeof(buff));
    if (n > 0)
    {
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 332 "winapi.l.c"
    lua_pushboolean(L, SetForegroundWindow(thisW->hwnd));
    return 1;
}
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 339 "winapi.l.c"
    DWORD pid;
    GetWindowThreadProcessId(thisW->hwnd, &pid);
...
{
    Window* thisW = Window_arg(L, 1);
    // OFF #line 347 "winapi.l.c"
    int ret;
    int sz = GetWindowTextW(thisW->hwnd, wbuff, sizeof(wbuff));
    if (sz > MAX_SHOW)
    {
        wbuff[MAX_SHOW] = '\0';
    }
...
static int l_get_foreground_window(lua_State* L) { return push_new_Window(L, GetForegroundWindow()); }

/// the desktop window.
// @usage winapi.get_desktop_window():get_bounds()
// @return @{Window}
// @function get_desktop_window
static int l_get_desktop_window(lua_State* L) { return push_new_Window(L, GetDesktopWindow()); }
...
static int l_send_to_window(lua_State* L)
{
    ...
    for (; *text; ++text)
    {
        ...
        if (vk & 0x100)
            pi = add_input(pi, VK_SHIFT, FALSE);
        pi = add_input(pi, vk & 0xFF, FALSE);
        if (vk & 0x100)
            pi = add_input(pi, VK_SHIFT, TRUE);
        last_vk = vk;
    }
...
// @param icon (default 'information') one of
...
static int l_copy_file(lua_State* L)
{
    ...
    // OFF #line 596 "winapi.l.c"
    return push_bool(L, CopyFile(src, dest, fail_if_exists));
}

/// output text to the system debugger.
// A uility such as
// [DebugView](http://technet.microsoft.com/en-us/sysinternals/bb896647) can
// show the output
// @param str text
...
static int l_get_clipboard(lua_State* L)
{
    ...
    p = static_cast<LPCWSTR>(GlobalLock(glob)); // tex cast
    push_wstring(L, p);
    GlobalUnlock(glob);
    CloseClipboard();
    return 1;
...
// @param defn a string as used by the [mode
// command](http://technet.microsoft.com/en-us/library/cc732236%28WS.10%29.aspx)
// @return @{File}
// @function open_serial
static int l_open_serial(lua_State* L)
{
    const char* defn = luaL_checklstring(L, 1, NULL);
    // OFF #line 701 "winapi.l.c"
    DCB dcb = {0};
    char port[20];
    HANDLE hSerial;
    const char* p = defn;
...
static int push_wait(lua_State* L, HANDLE h, int timeout) { return push_wait_result(L, wait_single(h, timeout)); }

static int push_wait_async(lua_State* L, HANDLE h, int timeout, int callback);

/// The Event class.
// @type Event
// OFF #line 761 "winapi.l.c"
...
{
    Event* thisW = (Event*) luaL_checkudata(L, idx, Event_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Event expected");
    return thisW;
}

static void Event_ctor(lua_State* L, Event* thisW, HANDLE h);

static int push_new_Event(lua_State* L, HANDLE h)
{
    Event* thisW = (Event*) lua_newuserdata(L, sizeof(Event));
    luaL_getmetatable(L, Event_MT);
    lua_setmetatable(L, -2);
    Event_ctor(L, thisW, h);
    return 1;
}

static void Event_ctor(lua_State* L, Event* thisW, HANDLE h)
{
    // OFF #line 762 "winapi.l.c"
    thisW->hEvent = h;
}

/// wait for this event to be signalled.
// @param timeout optional timeout in millisec; defaults to waiting
// indefinitely.
...
{
    Event* thisW = Event_arg(L, 1);
    int timeout = luaL_optinteger(L, 2, 0);
    // OFF #line 771 "winapi.l.c"
    return push_wait(L, thisW->hEvent, TIMEOUT(timeout));
}
...
static int l_Event_wait_async(lua_State* L)
{
    ...
    // OFF #line 781 "winapi.l.c"
    return push_wait_async(L, thisW->hEvent, TIMEOUT(timeout), callback);
}

static int l_Event_signal(lua_State* L)
{
    Event* thisW = Event_arg(L, 1);
    // OFF #line 785 "winapi.l.c"
    SetEvent(thisW->hEvent);
    return 0;
}
...
{
    Event* thisW = Event_arg(L, 1);
    // OFF #line 790 "winapi.l.c"
    CloseHandle(thisW->hEvent);
    return 0;
}
// OFF #line 793 "winapi.l.c"

static const struct luaL_Reg Event_methods[] = {
    {"wait", l_Event_wait}, {"wait_async", l_Event_wait_async}, {"signal", l_Event_signal}, {"__gc", l_Event___gc}, {NULL, NULL} /* sentinel */
};
...
// OFF #line 795 "winapi.l.c"

/// The Mutex class.
// @type Mutex
// OFF #line 800 "winapi.l.c"
...
{
    Mutex* thisW = (Mutex*) luaL_checkudata(L, idx, Mutex_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Mutex expected");
    return thisW;
}

static void Mutex_ctor(lua_State* L, Mutex* thisW, HANDLE h);

static int push_new_Mutex(lua_State* L, HANDLE h)
{
    Mutex* thisW = (Mutex*) lua_newuserdata(L, sizeof(Mutex));
    luaL_getmetatable(L, Mutex_MT);
    lua_setmetatable(L, -2);
    Mutex_ctor(L, thisW, h);
    return 1;
}

static void Mutex_ctor(lua_State* L, Mutex* thisW, HANDLE h)
{
    // OFF #line 801 "winapi.l.c"
    thisW->hMutex = h;
}

static int l_Mutex_lock(lua_State* L)
{
    Mutex* thisW = Mutex_arg(L, 1);
    // OFF #line 805 "winapi.l.c"
    WaitForSingleObject(thisW->hMutex, INFINITE);
    return 0;
}
...
{
    Mutex* thisW = Mutex_arg(L, 1);
    // OFF #line 810 "winapi.l.c"
    ReleaseMutex(thisW->hMutex);
    return 0;
}
...
{
    Mutex* thisW = Mutex_arg(L, 1);
    // OFF #line 815 "winapi.l.c"
    CloseHandle(thisW->hMutex);
    return 0;
}
// OFF #line 818 "winapi.l.c"

static const struct luaL_Reg Mutex_methods[] = {
    {"lock", l_Mutex_lock}, {"release", l_Mutex_release}, {"__gc", l_Mutex___gc}, {NULL, NULL} /* sentinel */
};
...
{
    const char* name = luaL_optlstring(L, 1, "", NULL);
    // OFF #line 844 "winapi.l.c"
    return push_new_Mutex(L, CreateMutex(NULL, FALSE, *name == 0 ? NULL : name));
}

/// A class representing a Windows process.
// this example was
// [helpful](http://msdn.microsoft.com/en-us/library/ms682623%28VS.85%29.aspx)
// @type Process
// OFF #line 854 "winapi.l.c"
...
{
    Process* thisW = (Process*) luaL_checkudata(L, idx, Process_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Process expected");
    return thisW;
}

static void Process_ctor(lua_State* L, Process* thisW, Int pid, HANDLE ph);

static int push_new_Process(lua_State* L, Int pid, HANDLE ph)
{
    Process* thisW = (Process*) lua_newuserdata(L, sizeof(Process));
    luaL_getmetatable(L, Process_MT);
    lua_setmetatable(L, -2);
    Process_ctor(L, thisW, pid, ph);
    return 1;
}

static void Process_ctor(lua_State* L, Process* thisW, Int pid, HANDLE ph)
{
    // OFF #line 855 "winapi.l.c"
    if (ph)
    {
        thisW->pid = pid;
        thisW->hProcess = ph;
    ...
    {
        thisW->pid = pid;
        thisW->hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ | PROCESS_TERMINATE, FALSE, pid);
        if (!thisW->hProcess)
        {
            thisW->hProcess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid);
        }
    }
}
...
{
    Process* thisW = Process_arg(L, 1);
    int full = lua_toboolean(L, 2);
    // OFF #line 875 "winapi.l.c"
    HMODULE hMod;
    DWORD cbNeeded;
    wchar_t modname[MAX_PATH];

    if (EnumProcessModules(thisW->hProcess, &hMod, sizeof(hMod), &cbNeeded))
    {
        if (full)
        {
            GetModuleFileNameExW(thisW->hProcess, hMod, modname, sizeof(modname));
        }
        else
        {
            GetModuleBaseNameW(thisW->hProcess, hMod, modname, sizeof(modname));
        }
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 894 "winapi.l.c"
    lua_pushnumber(L, thisW->pid);
    return 1;
}
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 902 "winapi.l.c"
    TerminateProcess(thisW->hProcess, 0);
    return 0;
}
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 911 "winapi.l.c"
    SIZE_T minsize, maxsize;
    GetProcessWorkingSetSize(thisW->hProcess, &minsize, &maxsize);
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 922 "winapi.l.c"
    FILETIME create, exit, kernel, user, local;
    SYSTEMTIME time;
    GetProcessTimes(thisW->hProcess, &create, &exit, &kernel, &user);
...
static int l_Process_wait(lua_State* L)
{
    Process* thisW = Process_arg(L, 1);
    int timeout = luaL_optinteger(L, 2, 0);
    // OFF #line 966 "winapi.l.c"
    return push_wait(L, thisW->hProcess, TIMEOUT(timeout));
}
...
static int l_Process_wait_async(lua_State* L)
{
    ...
    int callback = 2;
    int timeout = luaL_optinteger(L, 3, 0);
    // OFF #line 976 "winapi.l.c"
    return push_wait_async(L, thisW->hProcess, TIMEOUT(timeout), callback);
}
...
static int l_Process_wait_for_input_idle(lua_State* L)
{
    Process* thisW = Process_arg(L, 1);
    int timeout = luaL_optinteger(L, 2, 0);
    // OFF #line 987 "winapi.l.c"
    return push_wait_result(L, WaitForInputIdle(thisW->hProcess, TIMEOUT(timeout)));
}
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 995 "winapi.l.c"
    DWORD code;
    GetExitCodeProcess(thisW->hProcess, &code);
...
{
    Process* thisW = Process_arg(L, 1);
    // OFF #line 1004 "winapi.l.c"
    CloseHandle(thisW->hProcess);
    thisW->hProcess = NULL;
    return 0;
}
...
static int l_get_current_process(lua_State* L) { return push_new_Process(L, 0, GetCurrentProcess()); }
...
// These functions are all run in background threads, and a little bit of poor
// man's OOP helps here. this is the base struct for describing threads with
// callbacks, which may have an associated buffer and handle.
...
/// Thread object. this is returned by the @{File:read_async} method and the
/// @{make_timer},
// @{make_pipe_server} and @{watch_for_file_changes} functions. Useful to kill a
// thread and free associated resources.
// @type Thread
// OFF #line 1169 "winapi.l.c"
...
{
    Thread* thisW = (Thread*) luaL_checkudata(L, idx, Thread_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Thread expected");
    return thisW;
}

static void Thread_ctor(lua_State* L, Thread* thisW, PLuaCallback lcb, HANDLE thread);

static int push_new_Thread(lua_State* L, PLuaCallback lcb, HANDLE thread)
{
    Thread* thisW = (Thread*) lua_newuserdata(L, sizeof(Thread));
    luaL_getmetatable(L, Thread_MT);
    lua_setmetatable(L, -2);
    Thread_ctor(L, thisW, lcb, thread);
    return 1;
}

static void Thread_ctor(lua_State* L, Thread* thisW, PLuaCallback lcb, HANDLE thread)
{
    // OFF #line 1170 "winapi.l.c"
    thisW->lcb = lcb;
    thisW->thread = thread;
}
...
{
    Thread* thisW = Thread_arg(L, 1);
    // OFF #line 1177 "winapi.l.c"
    return push_bool(L, SuspendThread(thisW->thread) >= 0);
}
...
{
    Thread* thisW = Thread_arg(L, 1);
    // OFF #line 1183 "winapi.l.c"
    return push_bool(L, ResumeThread(thisW->thread) >= 0);
}
...
{
    Thread* thisW = Thread_arg(L, 1);
    // OFF #line 1191 "winapi.l.c"
    BOOL ret = TerminateThread(thisW->thread, 1);
    lcb_free(thisW->lcb);
...
static int l_Thread_set_priority(lua_State* L)
{
    ...
    // OFF #line 1200 "winapi.l.c"
    return push_bool(L, SetThreadPriority(thisW->thread, p));
}
...
{
    Thread* thisW = Thread_arg(L, 1);
    // OFF #line 1206 "winapi.l.c"
    int res = GetThreadPriority(thisW->thread);
...
static int l_Thread_wait(lua_State* L)
{
    Thread* thisW = Thread_arg(L, 1);
    int timeout = luaL_optinteger(L, 2, 0);
    // OFF #line 1220 "winapi.l.c"
    return push_wait(L, thisW->thread, TIMEOUT(timeout));
}
...
static int l_Thread_wait_async(lua_State* L)
{
    ...
    int callback = 2;
    int timeout = luaL_optinteger(L, 3, 0);
    // OFF #line 1230 "winapi.l.c"
    return push_wait_async(L, thisW->thread, TIMEOUT(timeout), callback);
}

static int l_Thread___gc(lua_State* L)
{
    Thread* thisW = Thread_arg(L, 1);
    // OFF #line 1235 "winapi.l.c"
    //  lcb_free(thisW->lcb); concerned that thisW cd kick in prematurely!
    CloseHandle(thisW->thread);
    return 0;
}
// OFF #line 1239 "winapi.l.c"

static const struct luaL_Reg Thread_methods[] = {
    {"suspend", l_Thread_suspend}, {"resume", l_Thread_resume}, {"kill", l_Thread_kill}, {"set_priority", l_Thread_set_priority}, {"get_priority", l_Thread_get_priority}, {"wait", l_Thread_wait}, {"wait_async", l_Thread_wait_async}, {"__gc", l_Thread___gc}, {NULL, NULL} /* sentinel */
};
...
// @type File
// OFF #line 1268 "winapi.l.c"

typedef struct
{
    callback_data_ HANDLE hWrite;
...
{
    File* thisW = (File*) luaL_checkudata(L, idx, File_MT);
    luaL_argcheck(L, thisW != NULL, idx, "File expected");
    return thisW;
}

static void File_ctor(lua_State* L, File* thisW, HANDLE hread, HANDLE hwrite);

static int push_new_File(lua_State* L, HANDLE hread, HANDLE hwrite)
{
    File* thisW = (File*) lua_newuserdata(L, sizeof(File));
    luaL_getmetatable(L, File_MT);
    lua_setmetatable(L, -2);
    File_ctor(L, thisW, hread, hwrite);
    return 1;
}

static void File_ctor(lua_State* L, File* thisW, HANDLE hread, HANDLE hwrite)
{
    // OFF #line 1269 "winapi.l.c"
    lcb_handle(thisW) = hread;
    thisW->hWrite = hwrite;
    thisW->L = L;
    lcb_allocate_buffer(thisW, FILE_BUFF_SIZE);
}
...
{
    File* thisW = File_arg(L, 1);
    const char* s = luaL_checklstring(L, 2, NULL);
    // OFF #line 1280 "winapi.l.c"
    DWORD bytesWrote;
    WriteFile(thisW->hWrite, s, lua_objlen(L, 2), &bytesWrote, NULL);
    lua_pushinteger(L, bytesWrote);
    return 1;
}

static BOOL raw_read(File* thisW)
{
    DWORD bytesRead = 0;
    BOOL res = ReadFile(lcb_handle(thisW), lcb_buf(thisW), lcb_bufsz(thisW), &bytesRead, NULL);
    lcb_buf(thisW)[bytesRead] = '\0';
...
{
    File* thisW = File_arg(L, 1);
    // OFF #line 1299 "winapi.l.c"
    if (raw_read(thisW))
    {
        lua_pushstring(L, lcb_buf(thisW));
        return 1;
    }
    else
    {
...
static void file_reader(File* thisW)
{ // background reader thread
    ...
    do
    {
        ...
        lcb_call(thisW, 0, lcb_buf(thisW), n == 0 ? DISCARD : 0);
    } while (n);
}
...
static int l_File_read_async(lua_State* L)
{
    File* thisW = File_arg(L, 1);
    int callback = 2;
    // OFF #line 1323 "winapi.l.c"
    thisW->callback = make_ref(L, callback);
    return lcb_new_thread((TCB) &file_reader, thisW);
}

static int l_File_close(lua_State* L)
{
    File* thisW = File_arg(L, 1);
    // OFF #line 1328 "winapi.l.c"
    if (thisW->hWrite != lcb_handle(thisW))
        CloseHandle(thisW->hWrite);
    lcb_free(thisW);
    return 0;
}
...
{
    File* thisW = File_arg(L, 1);
    // OFF #line 1335 "winapi.l.c"
    free(thisW->buf);
    return 0;
}
// OFF #line 1338 "winapi.l.c"

static const struct luaL_Reg File_methods[] = {
    {"write", l_File_write}, {"read", l_File_read}, {"read_async", l_File_read_async}, {"close", l_File_close}, {"__gc", l_File___gc}, {NULL, NULL} /* sentinel */
};
...
static int l_spawn_process(lua_State* L)
{
    ...
    running = CreateProcessW(NULL, (LPWSTR) wstring(program), NULL, NULL, TRUE, CREATE_NEW_PROCESS_GROUP, NULL, wconv(dir), &si, &pi);
...
// thisW is like `os.execute()`, except that it works without ugly
...
// in thisW case the result is always UTF-8
// @return return code
...
typedef struct
{
    callback_data_ int msec;
} TimerData;

static void timer_thread(TimerData* data)
...
typedef struct
{
    callback_data_ const char* pipename;
} PipeServerParms;
...
static void pipe_server_thread(PipeServerParms* parms)
{
    ...
    {
        BOOL connected;
        HANDLE hPipe = CreateNamedPipe(parms->pipename,    // pipe named
...
// thisW goes into a background loop, and accepts client connections.
...
static int l_short_path(lua_State* L)
{
    ...
    // OFF #line 1584 "winapi.l.c"
    WCHAR wpath[MAX_WPATH];
    HANDLE hFile;
    int res;
    wconv(path);
    // if the file doesn't exist, then force its creation
    hFile = CreateFileW(wpath, GENERIC_WRITE, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    { // if we created it successfully, then close.
        CloseHandle(hFile);
    }
...
typedef struct
{
    callback_data_ DWORD how;
    DWORD subdirs;
} FileChangeParms;

static void file_change_thread(FileChangeParms* fc)
{ // background file monitor thread
    while (1)
    {
        ...
        if (!ReadDirectoryChangesW(lcb_handle(fc), lcb_buf(fc), lcb_bufsz(fc), fc->subdirs, fc->how, &bytes, NULL, NULL))
        {
        ...
        do
        {
            ...
            outchars = WideCharToMultiByte(get_encoding(), 0, pni->FileName,
                                           pni->FileNameLength / 2, // it's bytes, not number of characters!
                                           outbuff, sizeof(outbuff), NULL, NULL);
            if (outchars == 0)
            {
...
static int l_watch_for_file_changes(lua_State* L)
{
    ...
    int callback = 4;
    // OFF #line 1778 "winapi.l.c"
    FileChangeParms* fc = (FileChangeParms*) malloc(sizeof(FileChangeParms));
    lcb_callback(fc, L, callback);
    fc->how = how;
    fc->subdirs = subdirs;
    lcb_handle(fc) = CreateFileW(wstring(dir), FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_ALWAYS, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
{
    Regkey* thisW = (Regkey*) luaL_checkudata(L, idx, Regkey_MT);
    luaL_argcheck(L, thisW != NULL, idx, "Regkey expected");
    return thisW;
}

static void Regkey_ctor(lua_State* L, Regkey* thisW, HKEY k);

static int push_new_Regkey(lua_State* L, HKEY k)
{
    Regkey* thisW = (Regkey*) lua_newuserdata(L, sizeof(Regkey));
    luaL_getmetatable(L, Regkey_MT);
    lua_setmetatable(L, -2);
    Regkey_ctor(L, thisW, k);
    return 1;
}

static void Regkey_ctor(lua_State* L, Regkey* thisW, HKEY k)
{
    // OFF #line 1803 "winapi.l.c"
    thisW->key = k;
}
...
static int l_Regkey_set_value(lua_State* L)
{
    ...
    res = RegSetValueExW(thisW->key, wname, 0, type, data, sz);
...
{
    // tex OFF cannot convert argument 5 from 'void *' to 'LPBYTE'	(data to
    // wbuff)
    /*
  Regkey *thisW = Regkey_arg(L,1);
  const char *name = luaL_optlstring(L,2,"",NULL);
  //OFF #line 1851 "winapi.l.c"
  DWORD type,size = sizeof(wbuff);
  void *data = wbuff;
  if (RegQueryValueExW(thisW->key,wstring(name),0,&type,data,&size) !=
  ERROR_SUCCESS) { return push_error(L);
  }
  if (type == REG_BINARY) {
...
static int l_Regkey_delete_key(lua_State* L)
{
    Regkey* thisW = Regkey_arg(L, 1);
    const char* name = luaL_checklstring(L, 2, NULL);
    // OFF #line 1869 "winapi.l.c"
    if (RegDeleteKeyW(thisW->key, wstring(name)) == ERROR_SUCCESS)
    {
        lua_pushboolean(L, 1);
    }
    else
...
static int l_Regkey_get_keys(lua_State* L)
{
    ...
    {
        size = sizeof(wbuff);
        res = RegEnumKeyExW(thisW->key, i, wbuff, &size, NULL, NULL, NULL, NULL);
        if (res != ERROR_SUCCESS)
            break;
        push_wstring(L, wbuff);
        lua_rawseti(L, -2, i + 1);
...
{
    Regkey* thisW = Regkey_arg(L, 1);
    // OFF #line 1905 "winapi.l.c"
    RegCloseKey(thisW->key);
    thisW->key = NULL;
    return 0;
}
...
{
    Regkey* thisW = Regkey_arg(L, 1);
    // OFF #line 1915 "winapi.l.c"
    return push_bool(L, RegFlushKey(thisW->key));
}

static int l_Regkey___gc(lua_State* L)
{
    Regkey* thisW = Regkey_arg(L, 1);
    // OFF #line 1919 "winapi.l.c"
    if (thisW->key != NULL)
        RegCloseKey(thisW->key);
    return 0;
}

// OFF #line 1924 "winapi.l.c"

static const struct luaL_Reg Regkey_methods[] = {
    {"set_value", l_Regkey_set_value}, {"get_value", l_Regkey_get_value}, {"delete_key", l_Regkey_delete_key}, {"get_keys", l_Regkey_get_keys}, {"close", l_Regkey_close}, {"flush", l_Regkey_flush}, {"__gc", l_Regkey___gc}, {NULL, NULL} /* sentinel */
};
...
static const char* lua_code_block = ""
                                    ...
                                    "function winapi.remove_dir(dir,tree) return exec_cmd('rmdir '..  ((tree "
                                    "and '/S /Q') or ''),dir) end\n"
                                    ...
                                    "    local ret, text = winapi.execute('dir '..flags..' "
                                    "\"'..mask..'\"','unicode')\n"
                                    "    if ret ~= 0 then return nil,text end\n"
                                    "    return text:gmatch('[^\\r\\n]+')\n"
                                    "end\n"
                                    "function winapi.dirs(mask,subdirs) return winapi.files(mask,subdirs,'D') "
                                    "end\n";
static void load_lua_code(lua_State* L) { luaL_dostring(L, lua_code_block); }

// OFF #line 2045 "winapi.l.c"
 ...
 * VK_F11,
 * VK_F12,
 * FILE\_NOTIFY\_CHANGE\_FILE\_NAME  (these are input flags for
watch\_for\_file\_changes)
 * FILE\_NOTIFY\_CHANGE\_DIR\_NAME
 * FILE\_NOTIFY\_CHANGE\_LAST\_WRITE
 * FILE\_ACTION\_ADDED     (these describe the change: first argument of
callback)
 ...
 * FILE\_ACTION\_RENAMED\_NEW\_NAME
 ...
 @section constants
 */
// OFF #line 2094 "winapi.l.c"

// OFF #line 2096 "winapi.l.c"
...
// OFF #line 2164 "winapi.l.c"
static const luaL_Reg winapi_funs[] = {{"set_encoding", l_set_encoding},
