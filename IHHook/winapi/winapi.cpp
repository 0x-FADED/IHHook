...
const DWORD_PTR WIN_NOACTIVATE = (DWORD_PTR)SWP_NOACTIVATE, WIN_NOMOVE = (DWORD_PTR)SWP_NOMOVE, WIN_NOSIZE = (DWORD_PTR)SWP_NOSIZE,
                WIN_SHOWWINDOW = (DWORD_PTR)SWP_SHOWWINDOW, WIN_NOZORDER = (DWORD_PTR)SWP_NOZORDER, WIN_BOTTOM = (DWORD_PTR)HWND_BOTTOM,
                WIN_NOTOPMOST = (DWORD_PTR)HWND_NOTOPMOST, WIN_TOP = (DWORD_PTR)HWND_TOP, WIN_TOPMOST = (DWORD_PTR)HWND_TOPMOST;
...
static const struct luaL_Reg File_methods[] = {
    { "write", l_File_write }, { "read", l_File_read }, { "read_async", l_File_read_async },
    { "close", l_File_close }, { "__gc", l_File___gc }, { NULL, NULL } /* sentinel */
};
...
static int l_watch_for_file_changes(lua_State* L)
{
    ...
    fc->how = how;
    fc->subdirs = subdirs;
    lcb_handle(fc) = CreateFileW(
        wstring(dir), FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_ALWAYS, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
{
    // tex OFF cannot convert argument 5 from 'void *' to 'LPBYTE'	(data to wbuff)
    /*
    Regkey *thisW = Regkey_arg(L,1);
    ...
    void *data = wbuff;
    if (RegQueryValueExW(thisW->key,wstring(name),0,&type,data,&size) != ERROR_SUCCESS) {
      return push_error(L);
    }
    if (type == REG_BINARY) {
