/*
        ...
        IHHook hooks lua by function addresses (defined in lua/*_Addresses.h),
   using (macros wrapping) MH_Hook initialised in CreateHooks() below it also
   replaces the lua function declarations in the lua distro (using the
   FUNCPTRDEF macros) so other code can build against it. TODO: this is no
   longer true, they currently in func_typedefs In some cases uses actual lua
   lib implementation. See comments on CREATE_FUNCPTR entries in
   *_Creathooks.cpp.

        function signatures/patterns would be more robust to game updates /
   different game versions than straight addresses, but take a long time to
   search since IHHook is started on it's own thread game initialisation will
   continue, and IHHook wont be ready in time to start up IH properly. an
   alternative would be to do a hook to an early execution point of the game and
   init  ihhook there, but given the low rate of updates of the game it's better
   to stick with direct addresses, but have signatures documented as a backup
*/
...
// tex CULL lua C module (well C++ because I converted so it would play nice
// with my mixed hooks and definitions version of the lua api) extern int
...
// http://www.lua.org/manual/5.1/manual.html#lua_pcall (also see the other
// functions that call HandleLuaError)
...
// tex there seems to be other calls to newstate that don't have lua libraries
// added, might be good to log calls to this and see if/when it's used) not
// really doing much with this hook since I shifted to luaL_openlibs as the lua
// setup func, but it's kinda the start of lua init, in respect to the lua C
// api. DEBUGNOW why does this crash unless you call the original function
// immediately?
...
// tex may be better to hook the fox engine OpenLuawhatever that calls newstate
// and sets up the lua libraries but don't know fox OpenLuas return type
void __fastcall luaL_openlibsHook(lua_State* L)
{
    ...
    // tex: The fox modules wont be up by this point, so they have a seperate
    // ReplaceStubbedOutFox
...
// tex not doing anything with this, but it may be interesting to see everything
// that mgsv lua is loading. and dumping the buffer of stuff that's not from a
// lua file not doing error handling here as lua_loadHook has that
int luaL_loadbufferHook(lua_State* L, const char* buff, size_t size, const char* name)
{
    // spdlog::trace("luaL_loadbufferHook {}", name);//tex OFF since
    // lua_loadHook grabs it fine, but not diabling the hook in case I want to
    // breakpoint this on a whim spdlog::trace(buff);//TODO: dump stuff that's
    // not from a file
...
static void dumpstack(lua_State* L)
{
    ...
    for (int i = 1; i <= top; i++)
    {
        ...
        switch (lua_type(L, i))
        {
        case LUA_TNUMBER: luaLog->debug("{} number:\t {}", i, lua_tonumber(L, i)); break;
        case LUA_TSTRING: luaLog->debug("{} string:\t {}", i, lua_tostring(L, i)); break;
        case LUA_TBOOLEAN: luaLog->debug("{} bool:\t {}", i, (lua_toboolean(L, i) ? "true" : "false")); break;
        case LUA_TNIL: luaLog->debug("{} nil:\t nil", i); break;
        default: luaLog->debug("{} pointer:\t {}", i, lua_topointer(L, i)); break;
        }
    }
} // dumpstack

// tex retail build of MGSV stubs out a lot of functions (changes the function
// name > l_ function to point to same stubbed out function), unfortunately
// since they cut it off this way the only viable functions to replace are ones
// we already know about (like luaB_print, see ReplaceStubedOutFox below) see in
// exe the function base_funcs "print" points to, then see all other references
// to that func to see others that were treated that way Also DEBUGNOW it's
// being called before lua is even inited? only enabling hook after for now (the
// current ENABLEHOOK(l_StubbedOut) and commented out ENABLEHOOK(l_StubbedOut))
// DEBUGNOW there's a lot of uses of this replaced function that have alternate
...
static int l_StubbedOutHook(lua_State* L)
{
    ...
    // DEBUGNOW don't like this, this function is being called before lua is up
    // suggesting its stubbing out non lua stuff?
    if (luaState == NULL)
    {
        return 0;
    }
...
void SetupLog()
{
    ...
    luaLog = spdlog::basic_logger_st("lua", luaLogName); // tex st/single threaded since we want to preserver
                                                         // order, it's better performance, and we wont be
...
void CreateHooks()
{
    ...
    if (addressSet["luaL_openlibs"] == NULL || addressSet["lua_newstate"] == NULL || addressSet["lua_newthread"] == NULL || addressSet["lua_load"] == NULL || addressSet["luaL_loadbuffer"] == NULL || addressSet["lua_atpanic"] == NULL || addressSet["lua_error"] == NULL || addressSet["lua_pcall"] == NULL || addressSet["lua_cpcall"] == NULL || addressSet["l_StubbedOut"] == NULL)
    { // DEBUGNOW
    ...
    else
    {
        ...
        ENABLEHOOK(lua_atpanic) // tex works, but if you want to catch
                                // exceptions from this dll itself then it just
                                // trips here instead of near the actual problem
        ENABLEHOOK(lua_error)
        ENABLEHOOK(lua_pcall)
        ENABLEHOOK(lua_cpcall)
        // ENABLEHOOK(l_StubbedOut)//DEBUGNOW enabling after lua is init in
        // openlibs see l_StubbedOutHook
    } // if name##Addr != NULL
} // CreateHooks
...
// tex: replacement for MGSVs stubbed out "print", original lua implementation
// in lbaselib.c
...
static int FoxLog(spdlog::level::level_enum level, const char* levelName, lua_State* L)
{
...
static int l_Fox_Log(lua_State* L) { return FoxLog(spdlog::level::debug, "Log", L); }

static int l_Fox_Caution(lua_State* L) { return FoxLog(spdlog::level::debug, "Caution", L); }

static int l_Fox_Warning(lua_State* L) { return FoxLog(spdlog::level::warn, "Warning", L); }

static int l_Fox_Error(lua_State* L) { return FoxLog(spdlog::level::err, "Error", L); }

// game lua to IHHook callbacks>
// tex called inside-out from init.lua via IH, TODO maybe see where init is
// loaded to make this independant from IH
...
// KLUDGE: haven't got an early execution point figured out for when the Fox
// modules are done/up so this is called via IH > IHH.Init/l_FoxLua_Init which
// means on the off chance that Fox engine calls these functions via the lua C
// api before init is run. Extremely unlikely (they'd more likely call the C
// function that the lua functions were wrapping), but who knows. DEBUGNOW
...
static int OnPanic(lua_State* L)
{
    ...
    // tex was fprintf(stderr, "PANIC: unprotected error in call to Lua API
    // (%s)\n",errorMsg);
...
{
    // tex cant be in newstate or following functions (luaL_openlibs) or it will
    // recurse
    spdlog::debug(__func__);
